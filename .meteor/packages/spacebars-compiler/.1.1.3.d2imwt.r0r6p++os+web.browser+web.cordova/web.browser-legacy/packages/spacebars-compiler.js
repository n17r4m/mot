...
//   TRIPLE, INCLUSION, BLOCKOPEN, and ELSE.                                                           // 28
//                                                                                                     // 29
...
var starts = {                                                                                         // 60
  ESCAPE: /^\{\{(?=\{*\|)/,                                                                            // 61
  ELSE: makeStacheTagStartRegex(/^\{\{\s*else(\s+(?!\s)|(?=[}]))/i),                                   // 62
...
TemplateTag.parse = function (scannerOrString) {                                                       // 88
  ...
  var scanExpr = function (type) {                                                                     // 239
    ...
    if (type === 'INCLUSION' || type === 'BLOCKOPEN' || type === 'ELSE')                               // 241
      endType = 'DOUBLE';                                                                              // 242
                                                                                                       // 243
  ...
  if (type === 'BLOCKCOMMENT') {                                                                       // 300
    ...
    if (! run(ends.DOUBLE)) {                                                                          // 315
      tag = scanExpr(type);                                                                            // 316
    }                                                                                                  // 317
  } else if (type === 'ESCAPE') {                                                                      // 318
    var result = run(/^\{*\|/);                                                                        // 319
    tag.value = '{{' + result.slice(0, -1);                                                            // 320
  } else {                                                                                             // 321
    // DOUBLE, TRIPLE, BLOCKOPEN, INCLUSION                                                            // 322
    tag = scanExpr(type);                                                                              // 323
  }                                                                                                    // 324
                                                                                                       // 325
  return tag;                                                                                          // 326
};                                                                                                     // 327
                                                                                                       // 328
// Returns a SpacebarsCompiler.TemplateTag parsed from `scanner`, leaving scanner                      // 329
// at its original position.                                                                           // 330
//                                                                                                     // 331
// An error will still be thrown if there is not a valid template tag at                               // 332
// the current position.                                                                               // 333
TemplateTag.peek = function (scanner) {                                                                // 334
  var startPos = scanner.pos;                                                                          // 335
  var result = TemplateTag.parse(scanner);                                                             // 336
  scanner.pos = startPos;                                                                              // 337
  return result;                                                                                       // 338
};                                                                                                     // 339
                                                                                                       // 340
// Like `TemplateTag.parse`, but in the case of blocks, parse the complete                             // 341
// `{{#foo}}...{{/foo}}` with `content` and possible `elseContent`, rather                             // 342
// than just the BLOCKOPEN tag.                                                                        // 343
//                                                                                                     // 344
// In addition:                                                                                        // 345
//                                                                                                     // 346
// - Throws an error if `{{else}}` or `{{/foo}}` tag is encountered.                                   // 347
//                                                                                                     // 348
// - Returns `null` for a COMMENT.  (This case is distinguishable from                                 // 349
//   parsing no tag by the fact that the scanner is advanced.)                                         // 350
//                                                                                                     // 351
// - Takes an HTMLTools.TEMPLATE_TAG_POSITION `position` and sets it as the                            // 352
//   TemplateTag's `.position` property.                                                               // 353
//                                                                                                     // 354
// - Validates the tag's well-formedness and legality at in its position.                              // 355
TemplateTag.parseCompleteTag = function (scannerOrString, position) {                                  // 356
  var scanner = scannerOrString;                                                                       // 357
  if (typeof scanner === 'string')                                                                     // 358
    scanner = new HTMLTools.Scanner(scannerOrString);                                                  // 359
                                                                                                       // 360
  var startPos = scanner.pos; // for error messages                                                    // 361
  var result = TemplateTag.parse(scannerOrString);                                                     // 362
  if (! result)                                                                                        // 363
    return result;                                                                                     // 364
                                                                                                       // 365
  if (result.type === 'BLOCKCOMMENT')                                                                  // 366
    return null;                                                                                       // 367
                                                                                                       // 368
  if (result.type === 'COMMENT')                                                                       // 369
    return null;                                                                                       // 370
                                                                                                       // 371
  if (result.type === 'ELSE')                                                                          // 372
    scanner.fatal("Unexpected {{else}}");                                                              // 373
                                                                                                       // 374
  if (result.type === 'BLOCKCLOSE')                                                                    // 375
    scanner.fatal("Unexpected closing template tag");                                                  // 376
                                                                                                       // 377
  position = (position || TEMPLATE_TAG_POSITION.ELEMENT);                                              // 378
  if (position !== TEMPLATE_TAG_POSITION.ELEMENT)                                                      // 379
    result.position = position;                                                                        // 380
                                                                                                       // 381
  if (result.type === 'BLOCKOPEN') {                                                                   // 382
    // parse block contents                                                                            // 383
                                                                                                       // 384
    // Construct a string version of `.path` for comparing start and                                   // 385
    // end tags.  For example, `foo/[0]` was parsed into `["foo", "0"]`                                // 386
    // and now becomes `foo,0`.  This form may also show up in error                                   // 387
    // messages.                                                                                       // 388
    var blockName = result.path.join(',');                                                             // 389
                                                                                                       // 390
    var textMode = null;                                                                               // 391
      if (blockName === 'markdown' ||                                                                  // 392
          position === TEMPLATE_TAG_POSITION.IN_RAWTEXT) {                                             // 393
        textMode = HTML.TEXTMODE.STRING;                                                               // 394
      } else if (position === TEMPLATE_TAG_POSITION.IN_RCDATA ||                                       // 395
                 position === TEMPLATE_TAG_POSITION.IN_ATTRIBUTE) {                                    // 396
        textMode = HTML.TEXTMODE.RCDATA;                                                               // 397
      }                                                                                                // 398
      var parserOptions = {                                                                            // 399
        getTemplateTag: TemplateTag.parseCompleteTag,                                                  // 400
        shouldStop: isAtBlockCloseOrElse,                                                              // 401
        textMode: textMode                                                                             // 402
      };                                                                                               // 403
    result.content = HTMLTools.parseFragment(scanner, parserOptions);                                  // 404
                                                                                                       // 405
    if (scanner.rest().slice(0, 2) !== '{{')                                                           // 406
      scanner.fatal("Expected {{else}} or block close for " + blockName);                              // 407
                                                                                                       // 408
    var lastPos = scanner.pos; // save for error messages                                              // 409
    var tmplTag = TemplateTag.parse(scanner); // {{else}} or {{/foo}}                                  // 410
                                                                                                       // 411
    var lastElseContentTag = result;                                                                   // 412
    while (tmplTag.type === 'ELSE') {                                                                  // 413
      if (lastElseContentTag === null) {                                                               // 414
        scanner.fatal("Unexpected else after {{else}}");                                               // 415
      }                                                                                                // 416
                                                                                                       // 417
      if (tmplTag.path) {                                                                              // 418
        lastElseContentTag.elseContent = new TemplateTag;                                              // 419
        lastElseContentTag.elseContent.type = 'BLOCKOPEN';                                             // 420
        lastElseContentTag.elseContent.path = tmplTag.path;                                            // 421
        lastElseContentTag.elseContent.args = tmplTag.args;                                            // 422
        lastElseContentTag.elseContent.content = HTMLTools.parseFragment(scanner, parserOptions);      // 423
                                                                                                       // 424
        lastElseContentTag = lastElseContentTag.elseContent;                                           // 425
      }                                                                                                // 426
      else {                                                                                           // 427
        // parse {{else}} and content up to close tag                                                  // 428
        lastElseContentTag.elseContent = HTMLTools.parseFragment(scanner, parserOptions);              // 429
                                                                                                       // 430
        lastElseContentTag = null;                                                                     // 431
      }                                                                                                // 432
                                                                                                       // 433
      if (scanner.rest().slice(0, 2) !== '{{')                                                         // 434
        scanner.fatal("Expected block close for " + blockName);                                        // 435
                                                                                                       // 436
      lastPos = scanner.pos;                                                                           // 437
      tmplTag = TemplateTag.parse(scanner);                                                            // 438
    }                                                                                                  // 439
                                                                                                       // 440
    if (tmplTag.type === 'BLOCKCLOSE') {                                                               // 441
      var blockName2 = tmplTag.path.join(',');                                                         // 442
      if (blockName !== blockName2) {                                                                  // 443
        scanner.pos = lastPos;                                                                         // 444
        scanner.fatal('Expected tag to close ' + blockName + ', found ' +                              // 445
                      blockName2);                                                                     // 446
      }                                                                                                // 447
    } else {                                                                                           // 448
      scanner.pos = lastPos;                                                                           // 449
      scanner.fatal('Expected tag to close ' + blockName + ', found ' +                                // 450
                    tmplTag.type);                                                                     // 451
    }                                                                                                  // 452
  }                                                                                                    // 453
                                                                                                       // 454
  var finalPos = scanner.pos;                                                                          // 455
  scanner.pos = startPos;                                                                              // 456
  validateTag(result, scanner);                                                                        // 457
  scanner.pos = finalPos;                                                                              // 458
                                                                                                       // 459
  return result;                                                                                       // 460
};                                                                                                     // 461
                                                                                                       // 462
var isAtBlockCloseOrElse = function (scanner) {                                                        // 463
  // Detect `{{else}}` or `{{/foo}}`.                                                                  // 464
  //                                                                                                   // 465
  // We do as much work ourselves before deferring to `TemplateTag.peek`,                              // 466
  // for efficiency (we're called for every input token) and to be                                     // 467
  // less obtrusive, because `TemplateTag.peek` will throw an error if it                              // 468
  // sees `{{` followed by a malformed tag.                                                            // 469
  var rest, type;                                                                                      // 470
  return (scanner.peek() === '{' &&                                                                    // 471
          (rest = scanner.rest()).slice(0, 2) === '{{' &&                                              // 472
          /^\{\{\s*(\/|else\b)/.test(rest) &&                                                          // 473
          (type = TemplateTag.peek(scanner).type) &&                                                   // 474
          (type === 'BLOCKCLOSE' || type === 'ELSE'));                                                 // 475
};                                                                                                     // 476
                                                                                                       // 477
// Validate that `templateTag` is correctly formed and legal for its                                   // 478
// HTML position.  Use `scanner` to report errors. On success, does                                    // 479
// nothing.                                                                                            // 480
var validateTag = function (ttag, scanner) {                                                           // 481
                                                                                                       // 482
  if (ttag.type === 'INCLUSION' || ttag.type === 'BLOCKOPEN') {                                        // 483
    var args = ttag.args;                                                                              // 484
    if (ttag.path[0] === 'each' && args[1] && args[1][0] === 'PATH' &&                                 // 485
        args[1][1][0] === 'in') {                                                                      // 486
      // For slightly better error messages, we detect the each-in case                                // 487
      // here in order not to complain if the user writes `{{#each 3 in x}}`                           // 488
      // that "3 is not a function"                                                                    // 489
    } else {                                                                                           // 490
      if (args.length > 1 && args[0].length === 2 && args[0][0] !== 'PATH') {                          // 491
        // we have a positional argument that is not a PATH followed by                                // 492
        // other arguments                                                                             // 493
        scanner.fatal("First argument must be a function, to be called on " +                          // 494
                      "the rest of the arguments; found " + args[0][0]);                               // 495
      }                                                                                                // 496
    }                                                                                                  // 497
  }                                                                                                    // 498
                                                                                                       // 499
  var position = ttag.position || TEMPLATE_TAG_POSITION.ELEMENT;                                       // 500
  if (position === TEMPLATE_TAG_POSITION.IN_ATTRIBUTE) {                                               // 501
    if (ttag.type === 'DOUBLE' || ttag.type === 'ESCAPE') {                                            // 502
      return;                                                                                          // 503
    } else if (ttag.type === 'BLOCKOPEN') {                                                            // 504
      var path = ttag.path;                                                                            // 505
      var path0 = path[0];                                                                             // 506
      if (! (path.length === 1 && (path0 === 'if' ||                                                   // 507
                                   path0 === 'unless' ||                                               // 508
                                   path0 === 'with' ||                                                 // 509
                                   path0 === 'each'))) {                                               // 510
      ...
      scanner.fatal(ttag.type + " template tag is not allowed in an HTML attribute");                  // 514
    }                                                                                                  // 515
  } else if (position === TEMPLATE_TAG_POSITION.IN_START_TAG) {                                        // 516
    if (! (ttag.type === 'DOUBLE')) {                                                                  // 517
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                       //
var UglifyJSMinify = null;                                                                             // 1
if (Meteor.isServer) {                                                                                 // 2
  UglifyJSMinify = Npm.require('uglify-js').minify;                                                    // 3
}                                                                                                      // 4
                                                                                                       // 5
SpacebarsCompiler.parse = function (input) {                                                           // 6
                                                                                                       // 7
  var tree = HTMLTools.parseFragment(                                                                  // 8
    input,                                                                                             // 9
    { getTemplateTag: TemplateTag.parseCompleteTag });                                                 // 10
                                                                                                       // 11
  return tree;                                                                                         // 12
};                                                                                                     // 13
                                                                                                       // 14
SpacebarsCompiler.compile = function (input, options) {                                                // 15
  var tree = SpacebarsCompiler.parse(input);                                                           // 16
  return SpacebarsCompiler.codeGen(tree, options);                                                     // 17
};                                                                                                     // 18
                                                                                                       // 19
SpacebarsCompiler._TemplateTagReplacer = HTML.TransformingVisitor.extend();                            // 20
SpacebarsCompiler._TemplateTagReplacer.def({                                                           // 21
  visitObject: function (x) {                                                                          // 22
    if (x instanceof HTMLTools.TemplateTag) {                                                          // 23
                                                                                                       // 24
      // Make sure all TemplateTags in attributes have the right                                       // 25
      // `.position` set on them.  This is a bit of a hack                                             // 26
      // (we shouldn't be mutating that here), but it allows                                           // 27
      // cleaner codegen of "synthetic" attributes like TEXTAREA's                                     // 28
      // "value", where the template tags were originally not                                          // 29
      // in an attribute.                                                                              // 30
      if (this.inAttributeValue)                                                                       // 31
        x.position = HTMLTools.TEMPLATE_TAG_POSITION.IN_ATTRIBUTE;                                     // 32
                                                                                                       // 33
      return this.codegen.codeGenTemplateTag(x);                                                       // 34
    }                                                                                                  // 35
                                                                                                       // 36
    return HTML.TransformingVisitor.prototype.visitObject.call(this, x);                               // 37
  },                                                                                                   // 38
  visitAttributes: function (attrs) {                                                                  // 39
    if (attrs instanceof HTMLTools.TemplateTag)                                                        // 40
      return this.codegen.codeGenTemplateTag(attrs);                                                   // 41
                                                                                                       // 42
    // call super (e.g. for case where `attrs` is an array)                                            // 43
    return HTML.TransformingVisitor.prototype.visitAttributes.call(this, attrs);                       // 44
  },                                                                                                   // 45
  visitAttribute: function (name, value, tag) {                                                        // 46
    this.inAttributeValue = true;                                                                      // 47
    var result = this.visit(value);                                                                    // 48
    this.inAttributeValue = false;                                                                     // 49
                                                                                                       // 50
    if (result !== value) {                                                                            // 51
      // some template tags must have been replaced, because otherwise                                 // 52
      // we try to keep things `===` when transforming.  Wrap the code                                 // 53
      // in a function as per the rules.  You can't have                                               // 54
      // `{id: Blaze.View(...)}` as an attributes dict because the View                                // 55
      // would be rendered more than once; you need to wrap it in a function                           // 56
      // so that it's a different View each time.                                                      // 57
      return BlazeTools.EmitCode(this.codegen.codeGenBlock(result));                                   // 58
    }                                                                                                  // 59
    return result;                                                                                     // 60
  }                                                                                                    // 61
});                                                                                                    // 62
                                                                                                       // 63
SpacebarsCompiler.codeGen = function (parseTree, options) {                                            // 64
  // is this a template, rather than a block passed to                                                 // 65
  // a block helper, say                                                                               // 66
  var isTemplate = (options && options.isTemplate);                                                    // 67
  var isBody = (options && options.isBody);                                                            // 68
  var sourceName = (options && options.sourceName);                                                    // 69
                                                                                                       // 70
  var tree = parseTree;                                                                                // 71
                                                                                                       // 72
  // The flags `isTemplate` and `isBody` are kind of a hack.                                           // 73
  if (isTemplate || isBody) {                                                                          // 74
    // optimizing fragments would require being smarter about whether we are                           // 75
    // in a TEXTAREA, say.                                                                             // 76
    tree = SpacebarsCompiler.optimize(tree);                                                           // 77
  }                                                                                                    // 78
                                                                                                       // 79
  // throws an error if using `{{> React}}` with siblings                                              // 80
  new ReactComponentSiblingForbidder({sourceName: sourceName})                                         // 81
    .visit(tree);                                                                                      // 82
                                                                                                       // 83
  var codegen = new SpacebarsCompiler.CodeGen;                                                         // 84
  tree = (new SpacebarsCompiler._TemplateTagReplacer(                                                  // 85
    {codegen: codegen})).visit(tree);                                                                  // 86
                                                                                                       // 87
  var code = '(function () { ';                                                                        // 88
  if (isTemplate || isBody) {                                                                          // 89
    code += 'var view = this; ';                                                                       // 90
  }                                                                                                    // 91
  code += 'return ';                                                                                   // 92
  code += BlazeTools.toJS(tree);                                                                       // 93
  code += '; })';                                                                                      // 94
                                                                                                       // 95
  code = SpacebarsCompiler._beautify(code);                                                            // 96
                                                                                                       // 97
  return code;                                                                                         // 98
};                                                                                                     // 99
                                                                                                       // 100
SpacebarsCompiler._beautify = function (code) {                                                        // 101
  if (!UglifyJSMinify) {                                                                               // 102
    return code;                                                                                       // 103
  }                                                                                                    // 104
                                                                                                       // 105
  var result = UglifyJSMinify(code, {                                                                  // 106
    fromString: true,                                                                                  // 107
    mangle: false,                                                                                     // 108
    compress: false,                                                                                   // 109
    output: {                                                                                          // 110
      ...
      width: 80                                                                                        // 113
                                                                                                       ...
                                                                                                       // 116
  var output = result.code;                                                                            // 117
  // Uglify interprets our expression as a statement and may add a semicolon.                          // 118
  // Strip trailing semicolon.                                                                         // 119
  output = output.replace(/;$/, '');                                                                   // 120
  return output;                                                                                       // 121
};                                                                                                     // 122
                                                                                                       // 123
/////////////////////////////////////////////////////////////////////////////////////////////////////////
