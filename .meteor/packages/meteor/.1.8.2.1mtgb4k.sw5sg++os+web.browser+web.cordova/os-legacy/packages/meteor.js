...
var Future = Npm.require('fibers/future');                                                                         // 2
                                                                                                                   // 3
Meteor._noYieldsAllowed = function (f) {                                                                           // 4
  var savedYield = Fiber.yield;                                                                                    // 5
  Fiber.yield = function () {                                                                                      // 6
    throw new Error("Can't call yield in a noYieldsAllowed block!");                                               // 7
  };                                                                                                               // 8
  try {                                                                                                            // 9
    return f();                                                                                                    // 10
  } finally {                                                                                                      // 11
    Fiber.yield = savedYield;                                                                                      // 12
  }                                                                                                                // 13
};                                                                                                                 // 14
                                                                                                                   // 15
Meteor._DoubleEndedQueue = Npm.require('meteor-deque');                                                            // 16
                                                                                                                   // 17
// Meteor._SynchronousQueue is a queue which runs task functions serially.                                         // 18
// Tasks are assumed to be synchronous: ie, it's assumed that they are                                             // 19
// done when they return.                                                                                          // 20
//                                                                                                                 // 21
// It has two methods:                                                                                             // 22
//   - queueTask queues a task to be run, and returns immediately.                                                 // 23
//   - runTask queues a task to be run, and then yields. It returns                                                // 24
//     when the task finishes running.                                                                             // 25
//                                                                                                                 // 26
// It's safe to call queueTask from within a task, but not runTask (unless                                         // 27
// you're calling runTask from a nested Fiber).                                                                    // 28
//                                                                                                                 // 29
// Somewhat inspired by async.queue, but specific to blocking tasks.                                               // 30
// XXX break this out into an NPM module?                                                                          // 31
// XXX could maybe use the npm 'schlock' module instead, which would                                               // 32
//     also support multiple concurrent "read" tasks                                                               // 33
//                                                                                                                 // 34
Meteor._SynchronousQueue = function () {                                                                           // 35
  var self = this;                                                                                                 // 36
  // List of tasks to run (not including a currently-running task if any). Each                                    // 37
  // is an object with field 'task' (the task function to run) and 'future' (the                                   // 38
  // Future associated with the blocking runTask call that queued it, or null if                                   // 39
  // called from queueTask).                                                                                       // 40
  self._taskHandles = new Meteor._DoubleEndedQueue();                                                              // 41
  // This is true if self._run() is either currently executing or scheduled to                                     // 42
  // do so soon.                                                                                                   // 43
  self._runningOrRunScheduled = false;                                                                             // 44
  // During the execution of a task, this is set to the fiber used to execute                                      // 45
  // that task. We use this to throw an error rather than deadlocking if the                                       // 46
  // user calls runTask from within a task on the same fiber.                                                      // 47
  self._currentTaskFiber = undefined;                                                                              // 48
  // This is true if we're currently draining.  While we're draining, a further                                    // 49
  // drain is a noop, to prevent infinite loops.  "drain" is a heuristic type                                      // 50
  // operation, that has a meaning like unto "what a naive person would expect                                     // 51
  // when modifying a table from an observe"                                                                       // 52
  self._draining = false;                                                                                          // 53
};                                                                                                                 // 54
                                                                                                                   // 55
var SQp = Meteor._SynchronousQueue.prototype;                                                                      // 56
                                                                                                                   // 57
SQp.runTask = function (task) {                                                                                    // 58
  var self = this;                                                                                                 // 59
                                                                                                                   // 60
  ...
  fut.wait();                                                                                                      // 81
                                                                                                                   ...
                                                                                                                   // 83
SQp.queueTask = function (task) {                                                                                  // 84
  var self = this;                                                                                                 // 85
  self._taskHandles.push({                                                                                         // 86
    task: task,                                                                                                    // 87
    name: task.name                                                                                                // 88
  });                                                                                                              // 89
  self._scheduleRun();                                                                                             // 90
  // No need to block.                                                                                             // 91
};                                                                                                                 // 92
                                                                                                                   // 93
SQp.flush = function () {                                                                                          // 94
  var self = this;                                                                                                 // 95
  self.runTask(function () {});                                                                                    // 96
};                                                                                                                 // 97
                                                                                                                   // 98
SQp.safeToRunTask = function () {                                                                                  // 99
  var self = this;                                                                                                 // 100
  return Fiber.current && self._currentTaskFiber !== Fiber.current;                                                // 101
};                                                                                                                 // 102
                                                                                                                   // 103
SQp.drain = function () {                                                                                          // 104
  var self = this;                                                                                                 // 105
  if (self._draining)                                                                                              // 106
    return;                                                                                                        // 107
  if (!self.safeToRunTask())                                                                                       // 108
    return;                                                                                                        // 109
  self._draining = true;                                                                                           // 110
  while (! self._taskHandles.isEmpty()) {                                                                          // 111
    self.flush();                                                                                                  // 112
  }                                                                                                                // 113
  self._draining = false;                                                                                          // 114
};                                                                                                                 // 115
                                                                                                                   // 116
SQp._scheduleRun = function () {                                                                                   // 117
  var self = this;                                                                                                 // 118
  // Already running or scheduled? Do nothing.                                                                     // 119
  if (self._runningOrRunScheduled)                                                                                 // 120
    return;                                                                                                        // 121
                                                                                                                   // 122
  self._runningOrRunScheduled = true;                                                                              // 123
  setImmediate(function () {                                                                                       // 124
    Fiber(function () {                                                                                            // 125
      self._run();                                                                                                 // 126
    }).run();                                                                                                      // 127
  });                                                                                                              // 128
                                                                                                                   ...
                                                                                                                   // 130
SQp._run = function () {                                                                                           // 131
  var self = this;                                                                                                 // 132
                                                                                                                   // 133
  if (!self._runningOrRunScheduled)                                                                                // 134
    throw new Error("expected to be _runningOrRunScheduled");                                                      // 135
                                                                                                                   // 136
  if (self._taskHandles.isEmpty()) {                                                                               // 137
    // Done running tasks! Don't immediately schedule another run, but                                             // 138
    // allow future tasks to do so.                                                                                // 139
    self._runningOrRunScheduled = false;                                                                           // 140
    return;                                                                                                        // 141
  }                                                                                                                // 142
  var taskHandle = self._taskHandles.shift();                                                                      // 143
                                                                                                                   // 144
  // Run the task.                                                                                                 // 145
  self._currentTaskFiber = Fiber.current;                                                                          // 146
  var exception = undefined;                                                                                       // 147
  try {                                                                                                            // 148
    taskHandle.task();                                                                                             // 149
  } catch (err) {                                                                                                  // 150
    if (taskHandle.future) {                                                                                       // 151
      // We'll throw this exception through runTask.                                                               // 152
      exception = err;                                                                                             // 153
    } else {                                                                                                       // 154
      Meteor._debug("Exception in queued task: " + (err.stack || err));                                            // 155
    }                                                                                                              // 156
  }                                                                                                                // 157
  self._currentTaskFiber = undefined;                                                                              // 158
                                                                                                                   // 159
  // Soon, run the next task, if there is any.                                                                     // 160
  self._runningOrRunScheduled = false;                                                                             // 161
  self._scheduleRun();                                                                                             // 162
                                                                                                                   // 163
  // If this was queued with runTask, let the runTask call return (throwing if                                     // 164
  // the task threw).                                                                                              // 165
  if (taskHandle.future) {                                                                                         // 166
    if (exception)                                                                                                 // 167
      taskHandle.future['throw'](exception);                                                                       // 168
    else                                                                                                           // 169
      taskHandle.future['return']();                                                                               // 170
  }                                                                                                                // 171
};                                                                                                                 // 172
                                                                                                                   // 173
// Sleep. Mostly used for debugging (eg, inserting latency into server                                             // 174
// methods).                                                                                                       // 175
//                                                                                                                 // 176
Meteor._sleepForMs = function (ms) {                                                                               // 177
  var fiber = Fiber.current;                                                                                       // 178
  setTimeout(function() {                                                                                          // 179
    fiber.run();                                                                                                   // 180
  }, ms);                                                                                                          // 181
  Fiber.yield();                                                                                                   // 182
};                                                                                                                 // 183
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                                   //
Meteor.startup = function startup(callback) {                                                                      // 1
  if (process.env.METEOR_PROFILE) {                                                                                // 2
    // Create a temporary error to capture the current stack trace.                                                // 3
    var error = new Error("Meteor.startup");                                                                       // 4
                                                                                                                   // 5
    // Capture the stack trace of the Meteor.startup call, excluding the                                           // 6
    // startup stack frame itself.                                                                                 // 7
    Error.captureStackTrace(error, startup);                                                                       // 8
                                                                                                                   // 9
    callback.stack = error.stack                                                                                   // 10
      .split(/\n\s*/) // Split lines and remove leading whitespace.                                                // 11
      .slice(0, 2) // Only include the call site.                                                                  // 12
      .join(" ") // Collapse to one line.                                                                          // 13
      .replace(/^Error: /, ""); // Not really an Error per se.                                                     // 14
  }                                                                                                                // 15
                                                                                                                   // 16
  var bootstrap = global.__meteor_bootstrap__;                                                                     // 17
  if (bootstrap &&                                                                                                 // 18
      bootstrap.startupHooks) {                                                                                    // 19
    bootstrap.startupHooks.push(callback);                                                                         // 20
  } else {                                                                                                         // 21
    // We already started up. Just call it now.                                                                    // 22
    callback();                                                                                                    // 23
  }                                                                                                                // 24
};                                                                                                                 // 25
                                                                                                                   // 26
...
Meteor.EnvironmentVariable = function () {                                                                         // 15
  this.slot = nextSlot++;                                                                                          // 16
                                                                                                                   ...
                                                                                                                   // 18
var EVp = Meteor.EnvironmentVariable.prototype;                                                                    // 19
                                                                                                                   // 20
EVp.get = function () {                                                                                            // 21
  Meteor._nodeCodeMustBeInFiber();                                                                                 // 22
                                                                                                                   // 23
  return Fiber.current._meteor_dynamics &&                                                                         // 24
    Fiber.current._meteor_dynamics[this.slot];                                                                     // 25
};                                                                                                                 // 26
                                                                                                                   // 27
// Most Meteor code ought to run inside a fiber, and the                                                           // 28
// _nodeCodeMustBeInFiber assertion helps you remember to include appropriate                                      // 29
// bindEnvironment calls (which will get you the *right value* for your                                            // 30
// environment variables, on the server).                                                                          // 31
//                                                                                                                 // 32
// In some very special cases, it's more important to run Meteor code on the                                       // 33
// server in non-Fiber contexts rather than to strongly enforce the safeguard                                      // 34
// against forgetting to use bindEnvironment. For example, using `check` in                                        // 35
// some top-level constructs like connect handlers without needing unnecessary                                     // 36
// Fibers on every request is more important that possibly failing to find the                                     // 37
// correct argumentChecker. So this function is just like get(), but it                                            // 38
// returns null rather than throwing when called from outside a Fiber. (On the                                     // 39
// client, it is identical to get().)                                                                              // 40
EVp.getOrNullIfOutsideFiber = function () {                                                                        // 41
  if (!Fiber.current)                                                                                              // 42
    return null;                                                                                                   // 43
  return this.get();                                                                                               // 44
};                                                                                                                 // 45
                                                                                                                   // 46
EVp.withValue = function (value, func) {                                                                           // 47
  Meteor._nodeCodeMustBeInFiber();                                                                                 // 48
                                                                                                                   // 49
  if (!Fiber.current._meteor_dynamics)                                                                             // 50
    Fiber.current._meteor_dynamics = [];                                                                           // 51
  var currentValues = Fiber.current._meteor_dynamics;                                                              // 52
                                                                                                                   // 53
  var saved = currentValues[this.slot];                                                                            // 54
  try {                                                                                                            // 55
    currentValues[this.slot] = value;                                                                              // 56
    var ret = func();                                                                                              // 57
  } finally {                                                                                                      // 58
    currentValues[this.slot] = saved;                                                                              // 59
  }                                                                                                                // 60
                                                                                                                   // 61
  return ret;                                                                                                      // 62
};                                                                                                                 // 63
...
Meteor.bindEnvironment = function (func, onException, _this) {                                                     // 84
  Meteor._nodeCodeMustBeInFiber();                                                                                 // 85
                                                                                                                   // 86
  var dynamics = Fiber.current._meteor_dynamics;                                                                   // 87
  var boundValues = dynamics ? dynamics.slice() : [];                                                              // 88
                                                                                                                   // 89
  if (!onException || typeof(onException) === 'string') {                                                          // 90
    var description = onException || "callback of async function";                                                 // 91
    onException = function (error) {                                                                               // 92
      Meteor._debug(                                                                                               // 93
        "Exception in " + description + ":",                                                                       // 94
        error && error.stack || error                                                                              // 95
      );                                                                                                           // 96
    };                                                                                                             // 97
  } else if (typeof(onException) !== 'function') {                                                                 // 98
    throw new Error('onException argument must be a function, string or undefined for Meteor.bindEnvironment().');
  }                                                                                                                // 100
                                                                                                                   // 101
  return function (/* arguments */) {                                                                              // 102
    var args = Array.prototype.slice.call(arguments);                                                              // 103
                                                                                                                   // 104
    var runWithEnvironment = function () {                                                                         // 105
      var savedValues = Fiber.current._meteor_dynamics;                                                            // 106
      try {                                                                                                        // 107
        // Need to clone boundValues in case two fibers invoke this                                                // 108
        // function at the same time                                                                               // 109
        Fiber.current._meteor_dynamics = boundValues.slice();                                                      // 110
        var ret = func.apply(_this, args);                                                                         // 111
      } catch (e) {                                                                                                // 112
        // note: callback-hook currently relies on the fact that if onException                                    // 113
        // throws and you were originally calling the wrapped callback from                                        // 114
        // within a Fiber, the wrapped call throws.                                                                // 115
        onException(e);                                                                                            // 116
      } finally {                                                                                                  // 117
        Fiber.current._meteor_dynamics = savedValues;                                                              // 118
      }                                                                                                            // 119
      return ret;                                                                                                  // 120
    };                                                                                                             // 121
                                                                                                                   // 122
    if (Fiber.current)                                                                                             // 123
      return runWithEnvironment();                                                                                 // 124
    Fiber(runWithEnvironment).run();                                                                               // 125
  };                                                                                                               // 126
};                                                                                                                 // 127
                                                                                                                   // 128
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
