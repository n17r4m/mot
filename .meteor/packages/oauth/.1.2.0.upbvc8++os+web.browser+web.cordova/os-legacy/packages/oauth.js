...
OAuth._generateState = function (loginStyle, credentialToken, redirectUrl) {                      // 59
  return Buffer.from(JSON.stringify({                                                             // 60
...
OAuth._stateFromQuery = function (query) {                                                        // 66
  var string;                                                                                     // 67
  try {                                                                                           // 68
    string = Buffer.from(query.state, 'base64').toString('binary');                               // 69
  } catch (e) {                                                                                   // 70
...
var renderEndOfLoginResponse = function (options) {                                               // 306
  ...
  var result = template.replace(/##CONFIG##/, JSON.stringify(config))                             // 345
    .replace(                                                                                     // 346
      /##ROOT_URL_PATH_PREFIX##/, __meteor_runtime_config__.ROOT_URL_PATH_PREFIX                  // 347
    );                                                                                            // 348
                                                                                                  // 349
  return "<!DOCTYPE html>\n" + result;                                                            // 350
};                                                                                                // 351
                                                                                                  // 352
// Writes an HTTP response to the popup window at the end of an OAuth                             // 353
// login flow. At this point, if the user has successfully authenticated                          // 354
// to the OAuth server and authorized this app, we communicate the                                // 355
// credentialToken and credentialSecret to the main window. The main                              // 356
// window must provide both these values to the DDP `login` method to                             // 357
// authenticate its DDP connection. After communicating these vaues to                            // 358
// the main window, we close the popup.                                                           // 359
//                                                                                                // 360
// We export this function so that developers can override this                                   // 361
// behavior, which is particularly useful in, for example, some mobile                            // 362
// environments where popups and/or `window.opener` don't work. For                               // 363
// example, an app could override `OAuth._endOfPopupResponse` to put the                          // 364
// credential token and credential secret in the popup URL for the main                           // 365
// window to read them there instead of using `window.opener`. If you                             // 366
// override this function, you take responsibility for writing to the                             // 367
// request and calling `res.end()` to complete the request.                                       // 368
//                                                                                                // 369
// Arguments:                                                                                     // 370
//   - res: the HTTP response object                                                              // 371
//   - details:                                                                                   // 372
//      - query: the query string on the HTTP request                                             // 373
//      - credentials: { token: *, secret: * }. If present, this field                            // 374
//        indicates that the login was successful. Return these values                            // 375
//        to the client, who can use them to log in over DDP. If                                  // 376
//        present, the values have been checked against a limited                                 // 377
//        character set and are safe to include in HTML.                                          // 378
//      - error: if present, a string or Error indicating an error that                           // 379
//        occurred during the login. This can come from the client and                            // 380
//        so shouldn't be trusted for security decisions or included in                           // 381
//        the response without sanitizing it first. Only one of `error`                           // 382
//        or `credentials` should be set.                                                         // 383
OAuth._endOfLoginResponse = function (res, details) {                                             // 384
  res.writeHead(200, {'Content-Type': 'text/html'});                                              // 385
                                                                                                  // 386
  var redirectUrl;                                                                                // 387
  if (details.loginStyle === 'redirect') {                                                        // 388
    redirectUrl = OAuth._stateFromQuery(details.query).redirectUrl;                               // 389
    var appHost = Meteor.absoluteUrl();                                                           // 390
    if (OAuth._checkRedirectUrlOrigin(redirectUrl)) {                                             // 391
      details.error = "redirectUrl (" + redirectUrl +                                             // 392
        ") is not on the same host as the app (" + appHost + ")";                                 // 393
      redirectUrl = appHost;                                                                      // 394
    }                                                                                             // 395
  }                                                                                               // 396
                                                                                                  // 397
  var isCordova = OAuth._isCordovaFromQuery(details.query);                                       // 398
                                                                                                  // 399
  if (details.error) {                                                                            // 400
    Log.warn("Error in OAuth Server: " +                                                          // 401
             (details.error instanceof Error ?                                                    // 402
              details.error.message : details.error));                                            // 403
    res.end(renderEndOfLoginResponse({                                                            // 404
      loginStyle: details.loginStyle,                                                             // 405
      setCredentialToken: false,                                                                  // 406
      redirectUrl: redirectUrl,                                                                   // 407
      isCordova: isCordova                                                                        // 408
    }), "utf-8");                                                                                 // 409
    return;                                                                                       // 410
  }                                                                                               // 411
                                                                                                  // 412
  // If we have a credentialSecret, report it back to the parent                                  // 413
  // window, with the corresponding credentialToken. The parent window                            // 414
  // uses the credentialToken and credentialSecret to log in over DDP.                            // 415
  res.end(renderEndOfLoginResponse({                                                              // 416
    loginStyle: details.loginStyle,                                                               // 417
    setCredentialToken: true,                                                                     // 418
    credentialToken: details.credentials.token,                                                   // 419
    credentialSecret: details.credentials.secret,                                                 // 420
    redirectUrl: redirectUrl,                                                                     // 421
    isCordova: isCordova                                                                          // 422
  }), "utf-8");                                                                                   // 423
};                                                                                                // 424
                                                                                                  // 425
                                                                                                  // 426
var OAuthEncryption = Package["oauth-encryption"] && Package["oauth-encryption"].OAuthEncryption;
                                                                                                  // 428
var usingOAuthEncryption = function () {                                                          // 429
  return OAuthEncryption && OAuthEncryption.keyIsLoaded();                                        // 430
};                                                                                                // 431
                                                                                                  // 432
// Encrypt sensitive service data such as access tokens if the                                    // 433
// "oauth-encryption" package is loaded and the oauth secret key has                              // 434
// been specified.  Returns the unencrypted plaintext otherwise.                                  // 435
//                                                                                                // 436
// The user id is not specified because the user isn't known yet at                               // 437
// this point in the oauth authentication process.  After the oauth                               // 438
// authentication process completes the encrypted service data fields                             // 439
// will be re-encrypted with the user id included before inserting the                            // 440
// service data into the user document.                                                           // 441
//                                                                                                // 442
OAuth.sealSecret = function (plaintext) {                                                         // 443
  if (usingOAuthEncryption())                                                                     // 444
    return OAuthEncryption.seal(plaintext);                                                       // 445
  else                                                                                            // 446
    return plaintext;                                                                             // 447
}                                                                                                 // 448
                                                                                                  // 449
// Unencrypt a service data field, if the "oauth-encryption"                                      // 450
// package is loaded and the field is encrypted.                                                  // 451
//                                                                                                // 452
// Throws an error if the "oauth-encryption" package is loaded and the                            // 453
// field is encrypted, but the oauth secret key hasn't been specified.                            // 454
//                                                                                                // 455
OAuth.openSecret = function (maybeSecret, userId) {                                               // 456
  if (!Package["oauth-encryption"] || !OAuthEncryption.isSealed(maybeSecret))                     // 457
    return maybeSecret;                                                                           // 458
                                                                                                  // 459
  return OAuthEncryption.open(maybeSecret, userId);                                               // 460
};                                                                                                // 461
                                                                                                  // 462
// Unencrypt fields in the service data object.                                                   // 463
//                                                                                                // 464
OAuth.openSecrets = function (serviceData, userId) {                                              // 465
  var result = {};                                                                                // 466
  _.each(_.keys(serviceData), function (key) {                                                    // 467
    result[key] = OAuth.openSecret(serviceData[key], userId);                                     // 468
  });                                                                                             // 469
  return result;                                                                                  // 470
};                                                                                                // 471
                                                                                                  // 472
