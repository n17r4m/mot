...
let execFileAsync;
module1.watch(require("../utils/processes.js"), {
  execFileAsync(v) {
    execFileAsync = v;
  }
...
let getRebuildArgs;
module1.watch(require("../static-assets/server/npm-rebuild-args.js"), {
  get(v) {
    getRebuildArgs = v;
  }
...
let wrapOptimistic;
module1.watch(require("optimism"), {
  wrap(v) {
    wrapOptimistic = v;
  }
...
meteorNpm.rebuildIfNonPortable = Profile("meteorNpm.rebuildIfNonPortable", function (nodeModulesDir) {
  ...
  dirsToRebuild.forEach(function (pkgPath) {
    const tempPkgDir = tempPkgDirs[pkgPath] = files.pathJoin(tempNodeModules, files.pathBasename(pkgPath)); // Copy the package directory instead of renaming it, so that the
...
const isPortable = Profile("meteorNpm.isPortable", dir => {
  ...
  const portableFile = files.pathJoin(dir, ".meteor-portable-" + portableVersion + ".json");
...
var runNpmCommand = meteorNpm.runNpmCommand = Profile("meteorNpm.runNpmCommand", function (args, cwd) {
  let getEnv;
  module1.watch(require("../cli/dev-bundle-bin-helpers.js"), {
    getEnv(v) {
      getEnv = v;
    }
...
function getInstalledDependenciesTree(dir) {
  function ls(nodeModulesDir) {
    ...
    contents.forEach(item => {
      ...
      if (from && utils.isNpmUrl(from) && !utils.isNpmUrl(info.version)) {
        info.version = from;
      }
...
const installNpmModule = meteorNpm.installNpmModule = (name, version, dir) => {
  const installArg = utils.isNpmUrl(version) ? version : `${name}@${version}`; // We don't use npm.commands.install since we couldn't figure out
  ...
  if (!result.success) {
    const pkgNotFound = `404 Not Found: ${utils.quotemeta(name)}@${utils.quotemeta(version)}`;
    const versionNotFound = "No matching version found for " + `${utils.quotemeta(name)}@${utils.quotemeta(version)}`;

    if (result.stderr.match(new RegExp(pkgNotFound))) {
      buildmessage.error(`there is no npm package named '${name}' in the npm registry`);
