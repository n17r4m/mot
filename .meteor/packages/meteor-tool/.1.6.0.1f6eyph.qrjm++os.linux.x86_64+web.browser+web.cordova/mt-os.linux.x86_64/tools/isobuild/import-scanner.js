...
let Script;
module1.watch(require("vm"), {
  Script(v) {
    Script = v;
  }

}, 2);
let isString, isEmpty, has, keys, each, map, omit;
...
let sha1;
module1.watch(require("../fs/watch.js"), {
  sha1(v) {
    sha1 = v;
  }
...
let archMatches;
module1.watch(require("../utils/archinfo.js"), {
  matches(v) {
    archMatches = v;
  }
...
module1.watch(require("./js-analyze.js"), {
...
let cssToCommonJS;
module1.watch(require("./css-modules.js"), {
  cssToCommonJS(v) {
    cssToCommonJS = v;
  }
...
let buildmessage;
module1.watch(require("../utils/buildmessage.js"), {
  default(v) {
    buildmessage = v;
  }
...
let LRU;
module1.watch(require("lru-cache"), {
  default(v) {
    LRU = v;
  }
...
let Profile;
module1.watch(require("../tool-env/profile.js"), {
  Profile(v) {
    Profile = v;
  }

}, 10);
let SourceNode, SourceMapConsumer;
module1.watch(require("source-map"), {
  SourceNode(v) {
    SourceNode = v;
  },
...
let pathJoin, pathRelative, pathNormalize, pathDirname, pathBasename, pathExtname, pathIsAbsolute, convertToOSPath, convertToPosixPath;
module1.watch(require("../fs/files.js"), {
  pathJoin(v) {
    pathJoin = v;
  },
...
let optimisticReadFile, optimisticStatOrNull, optimisticHashOrNull, shouldWatch;
module1.watch(require("../fs/optimistic.js"), {
...
let Resolver;
module1.watch(require("./resolver.js"), {
  default(v) {
    Resolver = v;
  }
...
const fakeSymbol = Symbol("fake"); // Default handlers for well-known file extensions.
...
const defaultExtensionHandlers = {
  ...
  ".json"(dataString) {
    ...
    return "module.exports = " + JSON.stringify(file.jsonData, null, 2) + ";\n";
  },
...
class ImportScanner {
  ...
  addInputFiles(files) {
    files.forEach(file => {
      this._checkSourceAndTargetPaths(file); // Note: this absolute path may not necessarily exist on the file
  ...
  _checkSourceAndTargetPaths(file) {
    ...
    if (file.targetPath !== file.sourcePath) {
      ...
      const absTargetPath = pathJoin(this.sourceRoot, file.targetPath); // If file.targetPath differs from file.sourcePath, generate a new
      ...
      const relativeId = this._getRelativeImportId(absSourcePath, absTargetPath); // Set the contents of the source module to import the target
      ...
      sourceFile.dataString += ["module.watch(require(" + JSON.stringify(relativeId) + "), {", '  "*": module.makeNsSetter()', "});", ""].join("\n");
  ...
  _combineFiles(oldFile, newFile) {
    ...
    function getChunk(file) {
      ...
      const node = consumer && SourceNode.fromStringWithSourceMap(file.dataString, consumer);
    ...
    oldFile.hash = sha1(oldFile.data); // If either oldFile or newFile has been imported non-dynamically,
    ...
    oldFile.imported = oldFile.imported === true || newFile.imported === true || oldFile.imported || newFile.imported;
  ...
  getOutputFiles() {
    ...
    return this.outputFiles.filter(file => {
      return file.installPath && !file[fakeSymbol] && (!file.lazy || file.imported === true || file.imported === "dynamic");
    });
  }
  ...
  _findImportedModuleIdentifiers(file) {
    ...
    const result = findImportedModuleIdentifiers(file.dataString, file.hash); // there should always be file.hash, but better safe than sorry
  ...
  _resolve(parentFile, id, forDynamicImport = false) {
    ...
    if (resolved && resolved.packageJsonMap) {
      ...
      each(resolved.packageJsonMap, (pkg, path) => {
        const packageJsonFile = this._addPkgJsonToOutput(path, pkg, forDynamicImport);
        ...
        const relativeId = this._getRelativeImportId(parentFile.installPath, packageJsonFile.installPath); // Although not explicitly imported, any package.json modules
  ...
  _getRelativeImportId(parentPath, childPath) {
    const relativeId = convertToPosixPath(pathRelative(pathDirname(parentPath), childPath), true); // If the result of pathRelative does not already start with a "." or
  ...
  _scanFile(file, forDynamicImport = false) {
    ...
    if (forDynamicImport && file.imported === "dynamic") {
    ...
    if (file.error) {
      ...
      buildmessage.error(file.error.message, file.error.info);
      return;
    }
    ...
    each(file.deps, (info, id) => {
      ...
      if (depFile) {
        ...
        if (depFile.jsonData && depFile.installPath.endsWith("/package.json") && depFile.implicit === true) {
      ...
      info.installPath = installPath; // If the module is not readable, _readModule may return
      ...
      depFile.lazy = true; // Setting depFile.imported = false is necessary so that
      // this._scanFile(depFile, dynamic) doesn't think the file has been
      // scanned already and return immediately.

      depFile.imported = false; // Append this file to the output array and record its index.

      this._addFile(absImportedPath, depFile); // On the server, modules in node_modules directories will be
      // handled natively by Node, so we don't need to build a
      // meteorInstall-style bundle beyond the entry-point module.


      if (!this.isWeb() && depFile.installPath.startsWith("node_modules/") && // If optimistic functions care about this file, e.g. because it
  ...
  _readModule(absPath) {
    ...
    const dataString = info.dataString; // Same logic/comment as stripBOM in node/lib/module.js:
    ...
    info.dataString = defaultExtensionHandlers[ext].call(info, info.dataString, info.hash);
  ...
  _getInstallPath(absPath) {
    let path = this._getNodeModulesInstallPath(absPath) || this._getSourceRootInstallPath(absPath);
  ...
  _addPkgJsonToOutput(pkgJsonPath, pkg, forDynamicImport = false) {
    ...
    if (file) {
      ...
      file.imported = forDynamicImport ? file.imported || "dynamic" : true;
      return file;
    }

    const data = Buffer.from(map(pkg, (value, key) => {
      const isIdentifier = /^[_$a-zA-Z]\w*$/.test(key);
      const prop = isIdentifier ? "." + key : "[" + JSON.stringify(key) + "]";
      return `exports${prop} = ${JSON.stringify(value)};\n`;
    }).join(""));
    const relPkgJsonPath = pathRelative(this.sourceRoot, pkgJsonPath);
    const pkgFile = {
      type: "js",
      // We represent the JSON module with JS.
      data,
      jsonData: pkg,
      deps: {},
      // Avoid accidentally re-scanning this file.
...
each(["_readFile", "_findImportedModuleIdentifiers", "_getInstallPath"], funcName => {
