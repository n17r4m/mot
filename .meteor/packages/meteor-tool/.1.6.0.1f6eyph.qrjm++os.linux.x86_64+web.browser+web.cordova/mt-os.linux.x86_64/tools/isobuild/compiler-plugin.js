...
let sha1, readAndWatchFileWithHash;
...
let sourceMapLength;
module1.watch(require("../utils/utils.js"), {
  sourceMapLength(v) {
    sourceMapLength = v;
  }
...
let Console;
module1.watch(require("../console/console.js"), {
  Console(v) {
    Console = v;
  }
...
let ImportScanner;
module1.watch(require("./import-scanner.js"), {
  default(v) {
    ImportScanner = v;
  }
...
class InputFile extends buildPluginModule.InputFile {
  ...
  getFileOptions() {
    var self = this; // XXX fileOptions only exists on some resources (of type "source"). The JS
  ...
  readAndWatchFileWithHash(path) {
    ...
    return readAndWatchFileWithHash(sourceBatch.unibuild.watchSet, osPath);
  }
...
class ResourceSlot {
    ...
    if (self.inputResource.type === "source") {
      if (sourceProcessor) {// If we have a sourceProcessor, it will handle the adding of the
        ...
        self.addJavaScript({
          ...
          bare: self.inputResource.fileOptions && (self.inputResource.fileOptions.bare || // XXX eventually get rid of backward-compatibility "raw" name
  ...
  addStylesheet(options) {
    ...
    if (useMeteorInstall && resource.lazy) {
      ...
      // when needed.
      resource.type = "js";
      resource.data = Buffer.from(cssToCommonJS(data, resource.hash), "utf8");
      self.jsOutputResources.push(resource);
    } else {
      ...
      resource.type = "css";
      resource.data = Buffer.from(data, 'utf8'), // XXX do we need to call convertSourceMapPaths here like we did
      //     in legacy handlers?
  ...
  addJavaScript(options) {
    const self = this; // #HardcodeJs this gets called by constructor in the "js" case
  ...
  addAsset(options) {
    ...
    if (!(options.data instanceof Buffer)) {
      if (_.isString(options.data)) {
        ...
        throw new Error("'data' option to addAsset must be a Buffer or " + "String: " + self.inputResource.path);
      }
    }
...
class PackageSourceBatch {
    ...
    unibuild.resources.forEach(function (resource) {
      ...
      if (resource.type === "source") {
        ...
        if (extension === null) {
          ...
          sourceProcessor = sourceProcessorSet.getByExtension(extension); // If resource.extension === 'js', it's ok for there to be no
  ...
  _getSourceProcessorSet() {
    ...
    const sourceProcessorSet = new buildPluginModule.SourceProcessorSet(isopack.displayName(), {
      hardcodeJs: true
    });
  ...
  static computeJsOutputFilesMap(sourceBatches) {
    ...
    map.get("modules").files.some(file => {
      ...
      map.forEach((info, name) => {
        if (!name) return;
        const mainModule = info.mainModule && `meteor/${name}/${info.mainModule.targetPath}`;
  ...
  getResources({
    ...
    resources.push(...this._linkJS(jsResources || flatten(_.pluck(this.resourceSlots, 'jsOutputResources')), this.useMeteorInstall && {
