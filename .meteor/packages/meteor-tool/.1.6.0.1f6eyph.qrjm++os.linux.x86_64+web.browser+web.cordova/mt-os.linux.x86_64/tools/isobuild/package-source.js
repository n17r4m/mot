...
let SourceArch;
module1.watch(require("./source-arch.js"), {
  default(v) {
    SourceArch = v;
  }
...
module1.watch(require("./package-namespace.js"), {
  PackageNamespace(v) {
    PackageNamespace = v;
  }
...
let PackageNpm;
module1.watch(require("./package-npm.js"), {
  PackageNpm(v) {
    PackageNpm = v;
  }
...
let PackageCordova;
module1.watch(require("./package-cordova.js"), {
  PackageCordova(v) {
    PackageCordova = v;
  }
...
let PackageAPI;
module1.watch(require("./package-api.js"), {
  PackageAPI(v) {
    PackageAPI = v;
  }
...
module1.watch(require("./test-files.js"), {
...
module1.watch(require("../utils/colon-converter.js"), {
  convert(v) {
    convertColonsInPath = v;
  }

}, 7);
let optimisticReadFile, optimisticHashOrNull, optimisticStatOrNull, optimisticReadMeteorIgnore;
...
_.extend(PackageSource.prototype, {
  ...
  _readAndWatchDirectory(relDir, watchSet, {
    include,
    exclude,
    names
  }) {
    ...
    _.each(compiler.ALL_ARCHES, function (arch) {
      ...
      var sourceArch = new SourceArch(self, {
        ...
        getFiles(sourceProcessorSet, watchSet) {
          ...
          return {
            sources: self._findSources(findOptions).sort(loadOrderSort(sourceProcessorSet, arch)).map(relPath => {
              return {
                relPath,
  ...
  _inferFileOptions(relPath, {
    ...
    const fileOptions = {};
    const isTest = global.testCommandMetadata && global.testCommandMetadata.isTest;
    const isAppTest = global.testCommandMetadata && global.testCommandMetadata.isAppTest;
    const isTestFile = (isTest || isAppTest) && isTestFilePath(relPath); // If running in test mode (`meteor test`), all files other than
    ...
    for (var i = 0; i < dirs.length; ++i) {
      ...
      if (dir === "node_modules") {
        fileOptions.lazy = true;
        fileOptions.transpile = false; // Return immediately so that we don't apply special meanings to
      ...
      if (i > 0 && dirs[i - 1] === "client" && dir === "compatibility" && isApp && // Skip this check for packages.
  ...
  _findSources({
    ...
    sourceReadOptions.exclude.push(...ignoreFiles); // Unless we're running tests, ignore all test filenames and if we are, ignore the
    ...
    const anyLevelExcludes = [/^tests\/$/, archinfo.matches(arch, "os") ? /^client\/$/ : /^server\/$/, ...sourceReadOptions.exclude];
    ...
    const nodeModulesReadOptions = _extends({}, sourceReadOptions, {
    ...
    function find(dir, depth, inNodeModules) {
      ...
      const readOptions = inNodeModules ? nodeModulesReadOptions : sourceReadOptions;
  ...
  _findAssets({
    ...
    if (!_.isEmpty(assetDirs)) {
      ...
      while (!_.isEmpty(assetDirs)) {
        dir = assetDirs.shift(); // remove trailing slash
