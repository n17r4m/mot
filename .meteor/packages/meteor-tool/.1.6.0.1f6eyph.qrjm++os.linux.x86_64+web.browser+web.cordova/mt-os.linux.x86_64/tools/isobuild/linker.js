...
var Module = function (options) {
  ...
  self.name = options.name || null; // files in the module. array of File
...
_.extend(Module.prototype, {
  ...
  getPrelinkedFiles: Profile("linker Module#getPrelinkedFiles", function () {
    var self = this; // If we don't want to create a separate scope for this module,
    ...
    if (self.useGlobalNamespace && !self.meteorInstallOptions) {
      ...
      return _.map(eagerFiles, function (file) {
        const cacheKey = JSON.stringify([file.sourceHash, file.bare, file.servePath]);
  ...
  _buildModuleTree(results, sourceWidth) {
    assert.ok(this.meteorInstallOptions); // Tree of File objects for all non-dynamic modules.
    ...
    _.each(this.files, file => {
      ...
      if (file.isDynamic()) {
        ...
        if (file.installPath.endsWith("/package.json") && file.jsonData) {
          const stub = {};
  ...
  _chunkifyModuleTree(tree, chunks, sourceWidth) {
    ...
    const chunksLengthBeforeWalk = chunks.length; // The tree of nested directories and module functions built above
  ...
  _stringifyInstallOptions() {
    let optionsString = JSON.stringify(this.meteorInstallOptions, null, 2);
...
var File = function (inputFile, module) {
  ...
  self.sourceHash = inputFile.hash || watch.sha1(self.source); // The path of the source file, relative to the root directory of the
  ...
  self.installPath = inputFile.installPath || null; // the path where this file would prefer to be served if possible

  self.servePath = inputFile.servePath; // Module identifiers imported or required by this module, if any.
...
_.extend(File.prototype, {
  ...
  getPrelinkedOutput: Profile("linker File#getPrelinkedOutput", function (options) {
    ...
    if (consumer && !noLineNumbers) {
      var padding = bannerPadding(bannerWidth); // We might have already done this split above.
      ...
      for (var i = 0, lineCount = lines.length; i < lineCount; ++i) {
        ...
        if (len < width && line[len - 1] !== "\\") {
          ...
          if (pos) {
            line += padding.slice(len, width) + " //"; // Not all source maps define a mapping for every line in the
