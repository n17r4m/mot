...
var EOL = require("os").EOL; // These two values (EXITING_MESSAGE and getInfoFile) must match the
...
Cp.reconnect = function reconnect(delay) {
  var self = this; // Display the "Server unavailable" warning only on the third attempt
...
Cp.connect = function connect() {
  ...
  fs.readFile(infoFile, "utf8", function (err, json) {
    ...
    self.setUpSocket(net.connect(info.port, "127.0.0.1"), info.key);
  });
};
...
Cp.setUpSocket = function setUpSocket(sock, key) {
  ...
  function onConnect() {
    ...
    self.reconnectCount = 0;
    self.connected = true; // Sending a JSON-stringified options object (even just an empty
...
function shellBanner() {
  var bannerLines = ["", "Welcome to the server-side interactive shell!"];
