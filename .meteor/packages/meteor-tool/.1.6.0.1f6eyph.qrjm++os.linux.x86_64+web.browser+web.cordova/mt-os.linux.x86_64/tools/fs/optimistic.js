...
let sha1;
module.watch(require("./watch.js"), {
  sha1(v) {
    sha1 = v;
  }

}, 4);
let pathSep, pathDirname, pathIsAbsolute, pathJoin, statOrNull, lstat, readFile, readdir, dependOnPath;
...
const optimisticStatOrNull = makeOptimistic("statOrNull", path => {
...
const optimisticLStatOrNull = makeOptimistic("lstatOrNull", path => {
  try {
...
const optimisticHashOrNull = makeOptimistic("hashOrNull", (...args) => {
  try {
...
const optimisticReadMeteorIgnore = wrap(dir => {
  ...
  if (meteorIgnoreStat && meteorIgnoreStat.isFile()) {
    return require("ignore")().add(optimisticReadFile(meteorIgnorePath, "utf8"));
  }
