...
function spawnMongod(mongodPath, port, dbPath, replSetName) {
  ...
  '--bind_ip', '127.0.0.1', '--port', port, '--dbpath', dbPath, // Use an 8MB oplog rather than 256MB. Uses less space on disk and
...
if (process.platform === 'win32') {
  ...
  findMongoPids = function (dbDir, port) {
    ...
    child_process.exec(psScript, // we don't want this to randomly fail just because you're running lots of
...
var launchMongo = function (options) {
  ...
  var initiateReplSetAndWaitForReady = function () {
    try {
      ...
      var db = new Db('meteor', new Server('127.0.0.1', options.port, {
        poolSize: 1,
        socketOptions: {
      ...
      let wasJustSecondary = false; // XXX timeout eventually?
      ...
      while (!stopped) {
        ...
        if (_.any(status.members, function (member) {
          return member.stateStr === 'STARTUP' || member.stateStr === 'STARTUP2' || member.stateStr === 'RECOVERING';
        })) {
          utils.sleepMs(20);
          continue;
        }

        const firstMemberState = status.members[0].stateStr; // Is the intended primary currently a secondary? (It passes through
        // that phase briefly.)
        ...
        for (var i = 1; i < status.members.length; ++i) {
          if (status.members[i].stateStr !== 'SECONDARY') {
            throw Error("Unexpected Mongo secondary status: " + JSON.stringify(status));
          }
        }
