...
let CordovaBuilder;
module1.watch(require("../cordova/builder.js"), {
  CordovaBuilder(v) {
    CordovaBuilder = v;
  }
...
module1.watch(require("../fs/safe-watcher.js"), {
  closeAllWatchers(v) {
    closeAllWatchers = v;
  }
...
let eachline;
module1.watch(require("../utils/eachline.js"), {
  eachline(v) {
    eachline = v;
  }
...
_.extend(AppProcess.prototype, {
  ...
  _computeEnvironment: function () {
    ...
    env.APP_ID = self.projectContext.appIdentifier; // We run the server behind our own proxy, so we need to increment
    ...
    env.METEOR_PRINT_ON_LISTEN = 'true'; // use node's path module and not 'files.js' because NODE_PATH is an
    // environment variable passed to an external process and needs to be
    // constructed in the OS-style.

    var path = require('path');

    env.NODE_PATH = self.nodePath.join(path.delimiter);
    return env;
  },
...
var AppRunner = function (options) {
  ...
  self._beforeStartPromise = null; // A hacky state variable that indicates that the proxy process (this process)
  ...
  self._refreshing = false; // Builders saved across rebuilds, so that targets can be re-written in
...
_.extend(AppRunner.prototype, {
  ...
  _runOnce: function (options) {
    ...
    if (cordovaRunner) {
      const pluginVersions = cordova.pluginVersionsFromStarManifest(bundleResult.starManifest);
    ...
    var runPromise = self.runPromise = self._makePromise("run"); // Run the program
    ...
    var setupClientWatcher = function () {
      ...
      clientWatcher = new watch.Watcher({
        ...
        onChange: function () {
          var outcome = watch.isUpToDate(serverWatchSet) ? 'changed-refreshable' // only a client asset has changed
    ...
    Console.enableProgressDisplay(false); // Wait for either the process to exit, or (if watchForChanges) a
    ...
    try {
      while (ret.outcome === 'changed-refreshable') {
        ...
        var oldPromise = self.runPromise = self._makePromise("run"); // Notify the server that new client assets have been added to the
        // build.


        self._refreshing = true; // ChildProcess.prototype.send used to be synchronous, but is now
        ...
        setupClientWatcher(); // Wait until another file changes.
