...
let request;
module1.watch(require("../utils/http-helpers.js"), {
  request(v) {
    request = v;
  }
...
let recordPackages;
module1.watch(require("./stats.js"), {
  recordPackages(v) {
    recordPackages = v;
  }
...
const CAPABILITIES = ['showDeployMessages', 'canTransferAuthorization']; // Make a synchronous RPC to the "classic" MDG deploy API. The deploy
...
function deployRpc(options) {
  ...
  try {
    var result = request(Object.assign(options, {
      url: deployURLBase + '/' + options.operation + (options.site ? '/' + options.site : ''),
...
function bundleAndDeploy(options) {
  ...
  var result = buildmessage.enterJob({
    title: "uploading"
  }, function () {
    return authedRpc({
      method: 'POST',
        ...
        if (err || cnames[0] !== 'origin.meteor.com') {
          dns.resolve(hostname, 'A', function (err, addresses) {
            if (err || addresses[0] !== '107.22.210.133') {
              Console.info('-------------');
              Console.info("You've deployed to a custom domain.", "Please be sure to CNAME your hostname", "to origin.meteor.com, or set an A record to 107.22.210.133.");
              Console.info('-------------');
            }
          });
        }
