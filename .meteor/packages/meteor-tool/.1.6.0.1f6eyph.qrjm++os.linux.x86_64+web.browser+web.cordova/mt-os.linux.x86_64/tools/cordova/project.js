...
let main;
module.watch(require("../cli/main.js"), {
  default(v) {
    main = v;
  }
...
module.watch(require("./protect-string-proto.js"));
let cordova_lib, cordova_events, CordovaError;
module.watch(require("cordova-lib"), {
  cordova(v) {
    cordova_lib = v;
  },
...
let cordova_util;
module.watch(require("cordova-lib/src/cordova/util.js"), {
  default(v) {
    cordova_util = v;
  }
...
let superspawn;
module.watch(require("cordova-common/src/superspawn.js"), {
  default(v) {
    superspawn = v;
  }
...
module.watch(require("cordova-common/src/PluginInfo/PluginInfoProvider.js"), {
  default(v) {
...
outdated platforms`); // Remove Cordova project directory to start afresh
    ...
    if (!files.exists(this.projectRoot)) {
      ...
      this.runCommands('creating Cordova project', () => Promise.asyncApply(() => {
        ...
        Promise.await(cordova_lib.raw.create(files.convertToOSPath(this.projectRoot), undefined, undefined, config));
      }), undefined, null);
    }
  } // Preparing
...
${displayNameForPlatform(platform)}`, () => Promise.asyncApply(() => {
      Promise.await(cordova_lib.raw.prepare(commandOptions));
    }));
...
${displayNameForPlatform(platform)}`, () => Promise.asyncApply(() => {
      Promise.await(cordova_lib.raw.build(commandOptions));
    }));
  } // Running


  run(platform, isDevice, options = [], extraPaths = []) {
...
${displayNameForPlatform(platform)} with options ${options}`, execFileAsync(command, options, {
        env: env,
...
requirements for platform ${displayNameForPlatform(platform)}`, () => Promise.asyncApply(() => {
      return Promise.await(cordova_lib.raw.requirements([platform], this.defaultOptions));
    }));
...
${displayNamesForPlatforms(platforms)}`, () => Promise.asyncApply(() => {
      Promise.await(cordova_lib.raw.platform('update', platforms, this.defaultOptions));
    }));
  }

  addPlatform(platform) {
    this.runCommands(`adding platform ${displayNameForPlatform(platform)} \
to Cordova project`, () => Promise.asyncApply(() => {
      let version = pinnedPlatformVersions[platform];
      let platformSpec = version ? `${platform}@${version}` : platform;
      Promise.await(cordova_lib.raw.platform('add', platformSpec, this.defaultOptions));
    }));
  }

  removePlatform(platform) {
    this.runCommands(`removing platform ${displayNameForPlatform(platform)} \
from Cordova project`, () => Promise.asyncApply(() => {
      Promise.await(cordova_lib.raw.platform('rm', platform, this.defaultOptions));
    }));
  }
...
to Cordova project`, () => Promise.asyncApply(() => {
        Promise.await(cordova_lib.raw.plugin('add', [target], commandOptions));
      }));
    }
...
from Cordova project`, () => Promise.asyncApply(() => {
      Promise.await(cordova_lib.raw.plugin('rm', plugins, this.defaultOptions));
    }));
...
convenience, but you should adjust your dependencies.`);
  ...
  runCommands(title, promiseOrAsyncFunction, env = this.defaultEnvWithPathsAdded(), cwd = this.projectRoot) {
...
running again with the --verbose option to help diagnose the issue.)`), consoleOptions);
      } else {
