...
let runLog;
module.watch(require("../runners/run-log.js"), {
  default(v) {
    runLog = v;
  }
...
class AndroidRunTarget extends CordovaRunTarget {
  ...
  tailLogs(cordovaProject, target) {
    return Promise.asyncApply(() => {
      const {
        transform
      } = require("../utils/eachline.js");

      cordovaProject.runCommands(`tailing logs for ${this.displayName}`, () => Promise.asyncApply(() => {
        Promise.await(this.checkPlatformRequirementsAndSetEnv(cordovaProject));
        const logLevel = Console.verbose ? "V" : "I";
        const filterExpressions = [`MeteorWebApp:${logLevel}`, `CordovaLog:${logLevel}`, `chromium:${logLevel}`, `SystemWebViewClient:${logLevel}`, '*:F'];
        const {
          Log
