...
let isString, isFunction, each, has;
...
let pathJoin, pathRelative, pathNormalize, pathDirname, convertToOSPath, convertToPosixPath;
...
class Resolver {
  ...
  _resolve(id, absParentPath, _seenDirPaths) {
    let resolved = this._resolveAbsolute(id, absParentPath) || this._resolveRelative(id, absParentPath) || this._resolveNodeModule(id, absParentPath);
  ...
  _resolvePkgJsonMain(dirPath, _seenDirPaths) {
    ...
    if (isString(main)) {
      ...
      const resolved = this._joinAndStat(dirPath, main) || this._resolve(main, pkgJsonPath, _seenDirPaths);
