...
let Builder;
module1.watch(require("./builder.js"), {
  default(v) {
    Builder = v;
  }
...
let JsFile, CssFile;
module1.watch(require("./minifier-plugin.js"), {
  JsFile(v) {
    JsFile = v;
  },
...
let addToTree;
module1.watch(require("./linker.js"), {
  addToTree(v) {
    addToTree = v;
  }
...
let loadIsopackage;
module1.watch(require("../tool-env/isopackets.js"), {
  loadIsopackage(v) {
    loadIsopackage = v;
  }
...
const SOURCE_URL_PREFIX = "meteor://\u{1f4bb}app"; // files to ignore when bundling. node has no globs, so use regexps
...
exports._mainJsContents = ["", "process.argv.splice(2, 0, 'program.json');", "process.chdir(require('path').join(__dirname, 'programs', 'server'));", "require('./programs/server/boot.js');"].join("\n"); ///////////////////////////////////////////////////////////////////////////////
...
class NodeModulesDirectory {
  ...
  static readDirsFromJSON(node_modules, _ref) {
    let {
      rebuildBinaries = false
    } = _ref,
        callerInfo = _objectWithoutProperties(_ref, ["rebuildBinaries"]);
    ...
    function add(moreInfo, path) {
      const info = _extends({}, callerInfo, moreInfo);
...
class File {
  constructor(options) {
    ...
    this.sourcePath = options.sourcePath; // info is just for help with debugging the tool; it isn't written to disk or
    // anything.
    ...
    this.url = null; // Is this file guaranteed to never change, so that we can let it be
  ...
  hash() {
    if (!this._hash) {
      ...
      this._hash = watch.sha1(String(File._salt()), this._hashOfContents);
    }
  ...
  setContents(b) {
    ...
    this._contents = b; // Un-cache hash.
...
class Target {
  ...
  make({
    ...
    buildmessage.enterJob("building for " + this.arch, () => {
      ...
      const sourceBatches = this._runCompilerPlugins(); // Link JavaScript and set up this.js, etc.
  ...
  _runCompilerPlugins() {
    ...
    const processor = new compilerPluginModule.CompilerPluginProcessor({
      ...
      linkerCacheDir: this.bundlerCacheDir && files.pathJoin(this.bundlerCacheDir, 'linker')
    });
  ...
  _emitResources(sourceBatches) {
    ...
    sourceBatches.forEach(sourceBatch => {
      ...
      const resources = sourceBatch.getResources(jsOutputFilesMap.get(name)); // First, find all the assets, so that we can associate them with each js
  ...
  minifyJs(minifierDef, minifyMode) {
    ...
    this.js.forEach(file => {
      ...
      if (file.targetPath.startsWith("dynamic/")) {
        ...
        file.hash(); // Dynamic files consist of a single anonymous function
        ...
        file._contents = Buffer.from(file.contents().toString("utf8").replace(/^\s*function\s*\(/, "function __minifyJs("), "utf8");
    ...
    function handle(source, dynamic) {
      source._minifiedFiles.forEach(file => {
        ...
        file.data = file.data.toString("utf8").replace(/^\s*function\s+__minifyJs\s*\(/, "function(");
  ...
  _addCordovaDependency(name, version, override) {
    ...
    if (override) {
      ...
      if (_.has(this.cordovaDependencies, name)) {
        ...
        this.cordovaDependencies[name] = packageVersionParser.lessThan(existingVersion, version) ? version : existingVersion;
      } else {
        this.cordovaDependencies[name] = version;
      }
    }
...
class ClientTarget extends Target {
  ...
  minifyCss(minifierDef, minifyMode) {
    ...
    const minifier = minifierDef.userPlugin.processFilesForBundle.bind(minifierDef.userPlugin);
  ...
  write(builder, {
    ...
    eachResource((file, type) => builder.reserve(file.targetPath)); // Build up a manifest of all resources served via HTTP.
    ...
    if (this.arch === 'web.cordova') {
      ...
      const cordovaCompatibilityVersions = _.object(_.map(CORDOVA_PLATFORM_VERSIONS, (version, platform) => {
        ...
        const hash = process.env[`METEOR_CORDOVA_COMPAT_VERSION_${platform.toUpperCase()}`] || WebAppHashing.calculateCordovaCompatibilityHash(version, cordovaDependencies);
...
class JsImage {
  ...
  load(bindings) {
    var self = this;
    var ret = {}; // XXX This is mostly duplicated from
      ...
      var env = _.extend({
        ...
        Npm: {
          require: Profile(function (name) {
            ...
            if (appNodeModules && tryLookup(appNodeModules, name)) {
              return require(fullPath);
            }
  ...
  write(builder, {
    ...
    _.each(self.jsToLoad, function (item) {
      ...
      if (item.sourceMap) {
        ...
        item.source = addSourceMappingURL(item.source, sourceMappingURL, item.targetPath);
...
class ServerTarget extends JsImageTarget {
  ...
  write(builder, {
    ...
    var self = this;
    var nodePath = []; // This is where the dev_bundle will be downloaded and unpacked

    builder.reserve('dependencies'); // Mapping from arch to relative path to the client program, if we have any
    ...
    _.each(["boot.js", "boot-utils.js", "debug.js", "server-json.js", "mini-files.js", "npm-require.js", "npm-rebuild.js", "npm-rebuild-args.js"], function (filename) {
      builder.write(filename, {
        file: files.pathJoin(toolsDir, 'static-assets', 'server', filename)
      });
    ...
    if (archinfo.VALID_ARCHITECTURES[self.arch] !== true) {
      throw new Error(`MDG does not publish dev_bundles for arch: ${self.arch}`);
...
function addSourceMappingURL(data, url, targetPath) {
  const parts = [// If data is a Buffer, convert it to a string.
  data.toString("utf8") // Remove any existing sourceURL or sourceMappingURL comments.
  .replace(/\n\/\/# source(?:Mapping)?URL=[^\n]+/g, '\n')];
...
var writeTargetToPath = Profile("bundler writeTargetToPath", function (name, target, outputPath, {
  includeNodeModules,
  getRelativeTargetPath,
  previousBuilder,
  buildMode,
...
var writeSiteArchive = Profile("bundler writeSiteArchive", function (targets, outputPath, {
  includeNodeModules,
  builtBy,
  releaseName,
  getRelativeTargetPath,
  ...
  try {
    ...
    var nodePath = []; // Tell the deploy server what version of the dependency kit we're using, so
...
`, 'utf8')
    ...
    Object.keys(targets).forEach(name => {
      ...
      const previousBuilder = previousBuilders && previousBuilders[name] ? previousBuilders[name] : null;
      const {
        arch,
        path,
        cordovaDependencies,
...
function bundle({
  ...
  var messages = buildmessage.capture({
    ...
    var makeServerTarget = Profile("bundler.bundle..makeServerTarget", function (app, clientTargets) {
      var targetOptions = {
        bundlerCacheDir,
        packageMap: projectContext.packageMap,
        isopackCache: projectContext.isopackCache,
    ...
    var getRelativeTargetPath = function (options) {
      ...
      return files.pathRelative(pathForTarget(options.relativeTo), pathForTarget(options.forTarget));
    }; // Write to disk
    ...
    if (outputPath !== null) {
      if (hasCachedBundle) {
        ...
        _.each(targets, function (target, name) {
          ...
          var targetBuild = writeTargetToPath(name, target, outputPath, _.extend({
            buildMode: buildOptions.buildMode
          }, writeOptions, {
            previousBuilder
          }));
        ...
        starResult = writeSiteArchive(targets, outputPath, _.extend({
          buildMode: buildOptions.buildMode
        }, writeOptions, {
          previousBuilders
        }));
