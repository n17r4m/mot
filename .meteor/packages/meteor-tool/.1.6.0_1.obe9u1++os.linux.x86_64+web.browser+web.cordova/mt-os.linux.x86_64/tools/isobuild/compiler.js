...
module1.watch(require("./build-plugin.js"), {
...
compiler.BUILT_BY = 'meteor/30'; // This is a list of all possible architectures that a build can target. (Client
...
var compileUnibuild = Profile(function (options) {
  ...
  _.values(assets).forEach(asset => {
    ...
    const absPath = files.pathResolve(inputSourceArch.sourceRoot, relPath); // readAndWatchFileWithHash returns an object carrying a buffer with the
  ...
  _.values(sources).forEach(source => {
    ...
    if (classification.type === 'unmatched') {
      ...
      buildmessage.error(`No plugin known to handle file '${relPath}'. If you want this \
file to be a static asset, use addAssets instead of addFiles; eg, \
api.addAssets('${relPath}', 'client').`); // recover by ignoring
...
function getActivePluginPackages(isopk, {
  uses,
  isopackCache,
  pluginProviderPackageNames,
