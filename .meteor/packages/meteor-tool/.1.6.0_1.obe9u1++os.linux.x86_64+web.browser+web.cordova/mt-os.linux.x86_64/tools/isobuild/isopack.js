let Builder;
module.watch(require("./builder.js"), {
  default(v) {
    Builder = v;
  }
...
module.watch(require("../utils/gc.js"), {
...
var Unibuild = function (isopack, options) {
  ...
  self.implies = options.implies || []; // This WatchSet will end up having the watch items from the
  ...
  self.watchSet = options.watchSet || new watch.WatchSet(); // Each Unibuild is given a unique id when it's loaded (it is
  ...
  self.id = self.pkg.name + "." + self.kind + "@" + self.arch + "#" + nextBuildId++; // 'declaredExports' are the variables which are exported from this package.
  // A list of objects with keys 'name' (required) and 'testOnly' (boolean,
  // defaults to false).

  self.declaredExports = options.declaredExports; // All of the data provided for eventual inclusion in the bundle,
  ...
  self.resources = options.resources; // Map from absolute paths of node_modules directories to
  // NodeModulesDirectory objects.

  self.nodeModulesDirectories = options.nodeModulesDirectories; // Provided for backwards compatibility; please use
...
_.extend(Isopack.prototype, {
    ...
    var Plugin = {
      ...
      _registerSourceProcessor: function ({
        ...
        if (!isopack.featureEnabled(featurePackage)) {
          ...
          buildmessage.error(`your package must \`api.use('${featurePackage}@1.0.0')\` in ` + `order for its plugins to call Plugin.${methodName}`);
          return;
        }
        ...
        if (extensions && extensions.some(e => e.endsWith('/'))) {
          buildmessage.error(`Plugin.${methodName}: extensions may not end in /`); // recover by ignoring

          return;
        }

        if (filenames && filenames.some(f => f.endsWith('/'))) {
          buildmessage.error(`Plugin.${methodName}: filenames may not end in /`); // recover by ignoring
  ...
  _loadUnibuildsFromPath: function (name, dir, options) {
    ...
    _.each(mainJson.builds, function (unibuildMeta) {
      ...
      var unibuildJson = JSON.parse(files.readFile(files.pathJoin(dir, unibuildMeta.path)));
      var unibuildBasePath = files.pathDirname(files.pathJoin(dir, unibuildMeta.path));
      ...
      var unibuildHasPrelink = unibuildJson.format === "unipackage-unibuild-pre1";
      var resources = [];

      _.each(unibuildJson.resources, function (resource) {
        rejectBadPath(resource.file);
        var data = files.readBufferWithLengthAndOffset(files.pathJoin(unibuildBasePath, resource.file), resource.length, resource.offset);

        if (resource.type === "prelink") {
          if (!unibuildHasPrelink) {
            throw Error("Unexpected prelink resource in " + unibuildJson.format + " at " + dir);
      ...
      const rebuildBinaries = archinfo.matches(archinfo.host(), unibuildMeta.arch);
      const nodeModulesDirectories = bundler.NodeModulesDirectory.readDirsFromJSON(unibuildJson.node_modules, {
  ...
  saveToPath: Profile("Isopack#saveToPath", function (outputDir, {
    ...
    try {
      ...
      _.each(self.unibuilds, function (unibuild) {
        ...
        var jsResourcesForLegacyPrelink = []; // Save unibuild dependencies. Keyed by the json path rather than thinking
        ...
        _.each(unibuild.nodeModulesDirectories, nmd => {
          ...
          ? npmDirsToCopy[nmd.sourcePath] : npmDirsToCopy[nmd.sourcePath] = builder.generateFilename(nmd.getPreferredBundlePath("isopack"), {
            directory: true
          });
        ...
        const usesModules = !isopackCache || isopackCache.uses(self, "modules", unibuild.arch); // Output 'head', 'body' resources nicely
        ...
        _.each(unibuild.resources, function (resource) {
          ...
          if (writeLegacyBuilds && resource.type === "source" && resource.extension == "js") {
  ...
  _writeTool: Profile("Isopack#_writeTool", function (builder) {
    ...
    'tools', 'examples', 'LICENSE.txt', 'LICENSES', 'meteor', 'meteor.bat', 'scripts/admin/launch-meteor', 'packages/package-version-parser/package-version-parser.js', 'packages/meteor/flush-buffers-on-exit-in-windows.js'); // Trim blank line and unnecessary examples.
    ...
    var transpileRegexes = [/^tools\/[^\/]+\.js$/, // General tools files
    ...
    /^tools\/packaging\/[^\/]+\.js$/, /^tools\/packaging\/catalog\/[^\/]+\.js$/, /^tools\/utils\/[^\/]+\.js$/, /^tools\/fs\/[^\/]+\.js$/, /^tools\/meteor-services\/[^\/]+\.js$/, /^tools\/tool-testing\/[^\/]+\.js$/, /^tools\/console\/[^\/]+\.js$/, /^tools\/cordova\/[^\/]+\.js$/]; // Split pathsToCopy into two arrays - one of files that should be copied
    ...
    pathsToCopy.forEach(path => {
      var shouldTranspile = _.some(transpileRegexes, regex => path.match(regex));
    ...
    builder = builder.enter(toolPath); // Transpile the files we selected

    var babel = require("meteor-babel");

    pathsToTranspile.forEach(path => {
      var fullPath = files.convertToOSPath(files.pathJoin(files.getCurrentToolsDir(), path));
      var inputFileContents = files.readFile(fullPath, "utf-8"); // #RemoveInProd
      ...
      if (path === "tools/tool-env/install-babel.js" || path === "tools/tool-env/source-map-retriever-stack.js") {
