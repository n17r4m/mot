...
function toArchArray(arch) {
  ...
  arch = _.uniq(arch);
  arch = _.map(arch, mapWhereToArch); // avoid using _.each so as to not add more frames to skip
...
function PackageAPI(options) {
  ...
  options = options || {};
  self.buildingIsopackets = !!options.buildingIsopackets; // source files used.
  // It's a multi-level map structured as:
  //   arch -> sources|assets -> relPath -> {relPath, fileOptions}

  self.files = {}; // symbols exported

  self.exports = {}; // packages used and implied (keys are 'package', 'unordered', and
  ...
  mainModule(path, arch, fileOptions = {}) {
    ...
    forAllMatchingArchs(arch, a => {
      ...
      const source = {
        ...
        fileOptions: _extends({}, fileOptions, {
          mainModule: true
        })
      };
