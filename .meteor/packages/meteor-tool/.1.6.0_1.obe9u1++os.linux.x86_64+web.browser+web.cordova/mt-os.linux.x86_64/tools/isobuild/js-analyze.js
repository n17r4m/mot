...
let parse;
module1.watch(require("meteor-babel"), {
  parse(v) {
    parse = v;
  }
...
var dependencyKeywordPattern = /\b(?:require|import|importSync|dynamicImport|export)\b/g;
/**
...
function findImportedModuleIdentifiers(source, hash) {
  ...
  function walk(node, left, right, requireIsBound) {
    if (left >= right) {// The window of possible indexes is empty, so we can ignore
      ...
      const end = node.end; // Narrow the left-right window to exclude possible indexes
      ...
      if (left < right) {
        if (!requireIsBound && isFunctionWithParameter(node, "require")) {
          requireIsBound = true;
        }
        ...
        if (importInfo) {
          return addIdentifier(importInfo.id, "import", requireIsBound, importInfo.dynamic);
  ...
  function addIdentifier(id, type, requireIsBound, isDynamic) {
    const entry = hasOwn.call(identifiers, id) ? identifiers[id] : identifiers[id] = {
    ...
    if (type === "require") {
      ...
      entry.possiblySpurious = entry.possiblySpurious && requireIsBound;
    } else {
...
function getRequiredModuleId(node) {
  if (node.type === "CallExpression" && isIdWithName(node.callee, "require")) {
...
function getImportedModuleInfo(node) {
  switch (node.type) {
    case "CallExpression":
      if (node.callee.type === "Import" || isIdWithName(node.callee, "import")) {
      ...
      } else if (node.callee.type === "MemberExpression" && isIdWithName(node.callee.object, "module")) {
        const propertyName = isPropertyWithName(node.callee.property, "import") || isPropertyWithName(node.callee.property, "importSync") || isPropertyWithName(node.callee.property, "dynamicImport");
