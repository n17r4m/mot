...
let safeWatcher;
module.watch(require("./safe-watcher.js"), {
  "*"(v) {
    safeWatcher = v;
  }
...
let coalesce;
module.watch(require("../utils/func-utils.js"), {
  coalesce(v) {
    coalesce = v;
  }
...
let Profile;
module.watch(require("../tool-env/profile.js"), {
  Profile(v) {
    Profile = v;
  }

}, 5);
let optimisticStatOrNull, optimisticReaddir, optimisticHashOrNull;
module.watch(require("./optimistic.js"), {
...
class WatchSet {
  constructor() {
    var self = this; // Set this to true if any Watcher built on this WatchSet must immediately
    // fire (eg, if this WatchSet was given two different sha1 for the same file).

    self.alwaysFire = false; // Map from the absolute path to a file, to a sha1 hash, or null if the file
    ...
    self.files = {}; // Array of object with keys:
  ...
  addFile(filePath, hash) {
    var self = this; // No need to update if this is in always-fire mode already.
  ...
  clone() {
    var self = this;
    var ret = new WatchSet(); // XXX doesn't bother to deep-clone the directory info
...
class Watcher {
  constructor(options) {
    var self = this; // The set to watch.
    ...
    self.watches = {// <absolute path of watched file or directory>: {
  ...
  _startFileWatches() {
    var self = this; // Set up a watch for each file
  ...
  _watchFileOrDirectory(absPath) {
    ...
    if (files.statOrNull(absPath)) {
      ...
      entry.watcher = safeWatcher.watch(absPath, onWatchEvent); // If we successfully created the watcher, invoke the callback
  ...
  _makeWatchEventCallback(absPath) {
    var self = this; // Sometimes we receive a rapid succession of change events, perhaps
    ...
    return coalesce(100, function onWatchEvent() {
      ...
      if (stat === null || stat.isFile()) {
        if (_.has(self.watchSet.files, absPath)) {
          self._fireIfFileChanged(absPath); // XXX #3335 We probably should check again in a second, due to low
        ...
        _.each(dirFiles, function (file) {
          var fullPath = files.pathJoin(absPath, file); // Recursively watch new files, if we ever previously tried to
        ...
        _.some(self.watchSet.directories, function (info) {
          return self.stopped || absPath === info.absPath && self._fireIfDirectoryChanged(info, true); // XXX #3335 We probably should check again in a second, due to low
          // filesystem modtime resolution.
        });
      }
    });
  ...
  stop() {
    var self = this;
    self.stopped = true; // Clean up file watches
...
function readAndWatchDirectory(watchSet, options) {
  ...
  watchSet.addDirectory(_.extend({
    contents: contents
  }, options));
  return contents;
}
