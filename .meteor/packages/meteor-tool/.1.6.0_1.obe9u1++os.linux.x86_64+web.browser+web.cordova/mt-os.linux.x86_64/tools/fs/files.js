...
var Profile = require('../tool-env/profile.js').Profile; // Attach all exports of miniFiles here to avoid code duplication
...
var nextStackFilenameCounter = 1; // Use the source maps specified to runJavaScript
...
sourceMapRetrieverStack.push(useParsedSourceMap); // Fibers are disabled by default for files.* operations unless
// process.env.METEOR_DISABLE_FS_FIBERS parses to a falsy value.

const YIELD_ALLOWED = !!(_.has(process.env, "METEOR_DISABLE_FS_FIBERS") && !JSON.parse(process.env.METEOR_DISABLE_FS_FIBERS));
...
files.getToolsVersion = function () {
  if (!files.inCheckout()) {
    ...
    if (files.exists(isopackJsonPath)) {
      ...
      parsed = JSON.parse(isopackJson); // XXX "isopack-1" is duplicate of isopack.currentFormat
...
files.getCurrentToolsDir = function () {
  return files.pathDirname(files.pathDirname(files.convertToStandardPath(__dirname)));
...
files.getSettings = function (filename, watchSet) {
  ...
  var str = buffer.toString('utf8'); // Ensure that the string is parseable in JSON, but there's no reason to use
...
function statOrNull(path, preserveSymlinks) {
  try {
    return preserveSymlinks ? files.lstat(path) : files.stat(path);
  } catch (e) {
    if (e.code === "ENOENT") {
      return null;
    }
...
files.rm_recursive_async = path => {
  return new Promise((resolve, reject) => {
    rimraf(files.convertToOSPath(path), err => err ? reject(err) : resolve());
  });
}; // Like rm -r.
...
files.blankHash = "47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU="; // Returns a base64 SHA256 hash representing a tree on disk. It is not sensitive
...
files.mkdir_p = function (dir, mode) {
  ...
  var success = files.mkdir_p(parentPath, mode); // parent is not a directory.
...
files.cp_r = function (from, to, options = {}) {
  ...
  if (stat.isDirectory()) {
    ...
    files.readdir(from).forEach(f => {
      if (options.ignore && _.any(options.ignore, pattern => f.match(pattern))) {
        return;
      }
      ...
      files.cp_r(fullFrom, files.pathJoin(to, f), options);
    });
    return;
  }
  ...
  if (stat.isSymbolicLink()) {
    ...
    if (options.transformContents) {
      files.writeFile(to, options.transformContents(files.readFile(from), files.pathBasename(from)), {
        mode
      });
    } else {
...
var copyFileHelper = function (from, to, mode) {
  ...
  var writeStream = files.createWriteStream(to, {
    mode: mode
  });
...
files.mkdtemp = function (prefix) {
  var make = function () {
    ...
    tmpDir = files.realpath(tmpDir); // make the directory. give it 3 tries in case of collisions from
    // crappy random.
...
files.extractTarGz = function (buffer, destPath, options) {
  ...
  var topLevelOfArchive = files.readdir(tempDir) // On Windows, the 7z.exe tool sometimes creates an auxiliary
  // PaxHeader directory.
...
function tryExtractWithNativeTar(buffer, tempDir, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const tarProc = spawn("tar", [flags, "-"], {
      ...
      stdio: options.verbose ? ["pipe", // Always need to write to tarProc.stdin.
      process.stdout, process.stderr] : "pipe"
    });
...
function tryExtractWithNative7z(buffer, tempDir, options) {
  ...
  const exeOSPath = files.convertToOSPath(files.pathJoin(files.getCurrentNodeBinDir(), "7z.exe"));
...
files.createTarGzStream = function (dirPath, options) {
  ...
  var zlib = require("zlib"); // Create a segment of the file path which we will look for to
  // identify exactly what we think is a "bin" file (that is, something
  // which should be expected to work within the context of an
  // 'npm run-script').


  var binPathMatch = ["", "node_modules", ".bin", ""].join(path.sep); // Don't use `{ path: dirPath, type: 'Directory' }` as an argument to
  ...
  var tarStream = fileStream.pipe(tar.Pack({
    noProprietary: true
  }));
...
files.renameDirAlmostAtomically = Profile("files.renameDirAlmostAtomically", (fromDir, toDir) => {
  const garbageDir = `${toDir}-garbage-${utils.randomToken()}`; // Get old dir out of the way, if it exists.
...
files.writeFileAtomically = Profile("files.writeFileAtomically", function (filename, contents) {
  const parentDir = files.pathDirname(filename);
  files.mkdir_p(parentDir);
  const tmpFile = files.pathJoin(parentDir, '.' + files.pathBasename(filename) + '.' + utils.randomToken());
...
files._generateScriptLinkToMeteorScript = function (scriptLocation) {
  ...
  var newScript = ["@echo off", "SETLOCAL", "SET METEOR_INSTALLATION=%~dp0%", // always convert to Windows path since this function can also be
...
files.linkToMeteorScript = function (scriptLocation, linkLocation, platform) {
  ...
  if (platform === 'win32') {
    ...
    files.writeFile(linkLocation, script, {
      encoding: "ascii"
    });
  } else {
    // Symlink meteor tool
...
function wrapFsFunc(fsFuncName, pathArgIndices, options) {
  ...
  function makeWrapper({
    ...
    function wrapper(...args) {
      ...
      const shouldBeSync = alwaysSync || sync; // There's some overhead in awaiting a Promise of an async call,
      ...
      const isQuickie = fsFuncName === 'stat' || fsFuncName === 'rename' || fsFuncName === 'symlink';
      const dirty = options && options.dirty;
      const dirtyFn = typeof dirty === "function" ? dirty : null;

      if (mayYield() && shouldBeSync && !isQuickie) {
  ...
  files[fsFuncName] = Profile('files.' + fsFuncName, makeWrapper({
    alwaysSync: true
  }));
  files.fsFixPath[fsFuncName] = Profile('wrapped.fs.' + fsFuncName, makeWrapper({
    sync: false
  }));
  files.fsFixPath[fsFuncName + 'Sync'] = Profile('wrapped.fs.' + fsFuncName + 'Sync', makeWrapper({
    sync: true
  }));
}
...
const dependOnPath = require("optimism").wrap( // Always return something different to prevent optimism from
...
wrapFsFunc("stat", [0]);
wrapFsFunc("lstat", [0]);
wrapDestructiveFsFunc("rename", [0, 1]); // After the outermost files.withCache call returns, the withCacheCache is
...
enableCache("stat");
enableCache("lstat"); // The fs.exists method is deprecated in Node v4:
...
wrapFsFunc("open", [0]); // XXX this doesn't give you the second argument to the callback

wrapFsFunc("read", []);
wrapFsFunc("write", []);
wrapFsFunc("close", []);
wrapFsFunc("symlink", [0, 1]);
wrapFsFunc("readlink", [0]); // These don't need to be Fiberized
...
files.readBufferWithLengthAndOffset = function (filename, length, offset) {
  var data = new Buffer(length); // Read the data from disk, if it is non-empty. Avoid doing IO for empty
