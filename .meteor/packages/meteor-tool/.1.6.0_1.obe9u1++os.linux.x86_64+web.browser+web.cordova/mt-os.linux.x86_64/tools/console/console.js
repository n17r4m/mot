...
var buildmessage = require('../utils/buildmessage.js'); // XXX: Are we happy with chalk (and its sub-dependencies)?
...
var FORCE_PRETTY = undefined; // Set the default CR to \r unless we're running with cmd

var CARRIAGE_RETURN = process.platform === 'win32' && process.stdout.isTTY && process.argv[1].toLowerCase().includes('cmd') ? new Array(249).join('\b') : '\r';
...
const STATUS_INTERVAL_MS = 50; // Message to show when we don't know what we're doing
...
class Console extends ConsoleBase {
  ...
  _wrapText(text, options) {
    options = options || {}; // Compute the maximum offset on the bulk of the message.
