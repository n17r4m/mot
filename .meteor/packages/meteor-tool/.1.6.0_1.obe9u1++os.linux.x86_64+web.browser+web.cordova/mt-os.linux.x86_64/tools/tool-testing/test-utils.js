...
var registrationUrlRegexp = /https:\/\/www\.meteor\.com\/setPassword\?([a-zA-Z0-9\+\/]+)/;
...
exports.ddpConnect = ddpConnect; // Given a registration token created by doing a deferred registration
...
exports.checkForSettings = selftest.markStack(function (appName, settings, timeoutSecs) {
  ...
  while (true) {
    ...
    var result = httpHelpers.request('http://' + appName); // XXX This is brittle; the test will break if we start formatting the
