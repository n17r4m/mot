...
module.watch(require("../utils/fiber-helpers.js"), {
...
let randomPort, randomToken, sleepMs, timeoutScaleFactor;
...
module.watch(require("../utils/parse-stack.js"), {
  markBottom(v) {
...
let archInfoHost;
module.watch(require("../utils/archinfo.js"), {
  host(v) {
    archInfoHost = v;
  }
...
module.watch(require("../meteor-services/config.js"), {
...
module.watch(require("../utils/http-helpers.js"), {
...
let Builder;
module.watch(require("../isobuild/builder.js"), {
  default(v) {
    Builder = v;
  }
...
let RemoteCatalog;
module.watch(require("../packaging/catalog/catalog-remote.js"), {
  RemoteCatalog(v) {
    RemoteCatalog = v;
  }
...
let IsopackCache;
module.watch(require("../isobuild/isopack-cache.js"), {
  IsopackCache(v) {
    IsopackCache = v;
  }
...
let loadIsopackage;
module.watch(require("../tool-env/isopackets.js"), {
  loadIsopackage(v) {
    loadIsopackage = v;
  }
...
let Tropohouse;
module.watch(require("../packaging/tropohouse.js"), {
  Tropohouse(v) {
    Tropohouse = v;
  }
...
let PackageMap;
module.watch(require("../packaging/package-map.js"), {
  PackageMap(v) {
    PackageMap = v;
  }
...
let releaseCurrent;
module.watch(require("../packaging/release.js"), {
  current(v) {
    releaseCurrent = v;
  }
...
let allUpgraders;
module.watch(require("../upgraders.js"), {
  allUpgraders(v) {
    allUpgraders = v;
  }
...
let execFileSync;
module.watch(require("../utils/processes.js"), {
  execFileSync(v) {
    execFileSync = v;
  }
...
module.watch(require("../cli/dev-bundle-helpers.js"), {
  ensureDependencies(v) {
    ensureDependencies = v;
  }

}, 22);
module.watch(require("../tool-env/install-runtime.js"));
module.watch(require("longjohn"));
...
function markStack(f) {
  return parseStackMarkTop(f);
}

const fail = markStack(function (reason) {
  throw new TestFailure(reason);
});
const expectEqual = markStack(function (actual, expected) {
...
const expectTrue = markStack(function (actual) {
  if (!actual) {
    throw new TestFailure('not-true');
  }
});
const expectFalse = markStack(function (actual) {
  if (actual) {
    throw new TestFailure('not-false');
  }
});
const expectThrows = markStack(function (f) {
  let threw = false;
...
'meteor-base', 'mobile-experience', 'mongo', 'blaze-html-templates', 'session', 'jquery', 'tracker', "autopublish", "insecure", "standard-minifier-css", "standard-minifier-js", "es5-shim", "shell-server"];
...
class OutputLog {
  ...
  forbid(pattern, channel) {
    this.lines.forEach(line => {
      ...
      if (match) {
        throw new TestFailure('forbidden-string-present', {
          run: this.run
        });
      }
    });
...
class Sandbox {
  ...
  createApp(to, template, options) {
    ...
    files.cp_r(files.pathJoin(files.convertToStandardPath(__dirname), '..', 'tests', 'apps', template), absoluteTo, {
      ignore: [/^local$/]
  ...
  _makeEnv() {
    ...
    if (process.env.TOOL_NODE_FLAGS && !process.env.SELF_TEST_TOOL_NODE_FLAGS) console.log('Consider setting SELF_TEST_TOOL_NODE_FLAGS to configure ' + 'self-test test applicaion spawns');
...
class Client {
  constructor(options) {
    ...
    this.url = "http://" + this.host + ":" + this.port + '/' + (Math.random() * 0x100000000 + 1).toString(36);
...
class PhantomClient extends Client {
  ...
  connect() {
    ...
    const scriptPath = files.pathJoin(files.getCurrentToolsDir(), "tools", "tool-testing", "phantom", "open-url.js");
    this.process = execFile(phantomPath, ["--load-images=no", files.convertToOSPath(scriptPath), this.url], {}, (error, stdout, stderr) => {
...
class BrowserStackClient extends Client {
  ...
  connect() {
    ...
    if (!browserStackKey) {
      throw new Error("BrowserStack key not found. Ensure that you " + "have installed your S3 credentials.");
    }
    ...
    this._launchBrowserStackTunnel(error => {
      ...
      this.driver = new this.npmPackageExports.Builder().usingServer('http://hub.browserstack.com/wd/hub').withCapabilities(capabilities).build();
  ...
  _launchBrowserStackTunnel(callback) {
    ...
    const args = [browserStackPath, browserStackKey, [this.host, this.port, 0].join(','), // Disable Live Testing and Screenshots, just test with Automate.
    '-onlyAutomate', // Do not wait for the server to be ready to spawn the process.
    '-skipCheck'];
    this.tunnelProcess = execFile('/usr/bin/env', ['bash', '-c', args.join(' ')]); // Called when the SSH tunnel is established.
...
function ensureBrowserStack() {
  const browserStackPath = files.pathJoin(files.getDevBundle(), 'bin', 'BrowserStackLocal');
  ...
  if (!browserStackStat) {
    ...
    enterJob("downloading BrowserStack binaries", () => {
      return new Promise((resolve, reject) => {
        const browserStackStream = files.createWriteStream(browserStackPath);
...
class Run {
  ...
  _endMatchers() {
    return this.matcherEndPromise = this.matcherEndPromise || Promise.all([this.stdoutMatcher.endAsync(), this.stderrMatcher.endAsync()]);
  }
    ...
    if (this.exitStatus === undefined) {
      ...
      const promise = new Promise((resolve, reject) => {
        ...
        timer = setTimeout(() => {
          ...
          reject(new TestFailure('exit-timeout', {
            run: this
          }));
        }, timeout * 1000);
      });
...
const wrapWithMarkStack = func => markStack(func); // `Run` class methods to wrap with `markStack`


['expectEnd', 'expectExit', 'forbid', 'forbidAll', 'forbidErr', 'match', 'matchBeforeExit', 'matchErr', 'matchErrBeforeExit', 'read', 'readErr', 'stop', 'tellMongo'].forEach(functionName => Run.prototype[functionName] = wrapWithMarkStack(Run.prototype[functionName])); ///////////////////////////////////////////////////////////////////////////////
// Defining tests
///////////////////////////////////////////////////////////////////////////////
...
const getAllTests = () => {
  ...
  filenames.forEach(n => {
    ...
    try {
      ...
      fileBeingLoadedHash = require('crypto').createHash('sha1').update(contents).digest('hex');
...
class TestList {
  ...
  notifyFailed(test, failureObject) {
    ...
    this.fileInfo[test.file].hasFailures = true; // Mark that the specific test failed.
  ...
  saveJUnitOutput(path) {
    ...
    Object.keys(grouped).forEach(file => {
      ...
      grouped[file].forEach(test => {
        const testCaseAttrs = [`name="${attrSafe(test.name)}"`];
        ...
        if (test.failed) {
          ...
          if (test.failureObject instanceof TestFailure) {
            countFailure++;
            failureElement = [`<error type="${test.failureObject.reason}">`, '<![CDATA[', inspect(test.failureObject.details, {
              depth: 4
            }), ']]>', '</error>'].join('\n');
          ...
          testCases.push([`<testcase ${testCaseAttrsString}>`, failureElement, '</testcase>'].join('\n'));
        } else {
          testCases.push(`<testcase ${testCaseAttrsString}/>`);
        }
      });
      const testSuiteAttrs = [`name="${file}"`, `tests="${testCases.length}"`, `failures="${countFailure}"`, `errors="${countError}"`, `time="${durationForOutput(this.durationMs)}"`];
      const testSuiteAttrsString = testSuiteAttrs.join(' ');
      testSuites.push([`<testsuite ${testSuiteAttrsString}>`, testCases.join('\n'), '</testsuite>'].join('\n'));
    });
...
function runTests(options) {
  ...
  function runTest(test, tries = 3) {
    ...
    if (failure) {
      ...
      if (--tries > 0) {
        Console.error("... retrying (" + tries + (tries === 1 ? " try" : " tries") + " remaining) ...", Console.options({
          indent: 2
        }));
      ...
      if (failure instanceof TestFailure) {
        ...
        const relpath = files.pathRelative(files.getCurrentToolsDir(), frames[0].file);
      ...
      Console.error("... ok (" + test.durationMs + " ms)", Console.options({
        indent: 2
      }));
    }
  }
