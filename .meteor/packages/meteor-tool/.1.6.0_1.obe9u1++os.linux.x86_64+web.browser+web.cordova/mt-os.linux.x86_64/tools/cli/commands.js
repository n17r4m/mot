...
let CordovaRunner;
module.watch(require("../cordova/runner.js"), {
  CordovaRunner(v) {
    CordovaRunner = v;
  }
...
module.watch(require("../cordova/run-targets.js"), {
  iOSRunTarget(v) {
    iOSRunTarget = v;
  },
...
module.watch(require("./example-repositories.js"), {
...
function parseMobileServerOption(mobileServerOption, optionName = 'mobile-server') {
...
main.registerCommand({
  ...
  if (options.list) {
    ...
    _.each(EXAMPLE_REPOSITORIES, function (repoInfo, name) {
      ...
      Console.info(Console.command(`${name}: ${repoInfo.repo}${branchInfo}`), Console.options({
        indent: 2
      }));
    });
  ...
  if (options.example) {
    ...
    Console.info(Console.command(`git clone ${repoInfo.repo}${branchOption}${path}`), Console.options({
      indent: 2
    }));
    return 0;
  }
  ...
  if (appPathAsEntered !== ".") {
    ...
    const appPathWithQuotesIfSpaces = appPathAsEntered.indexOf(' ') === -1 ? appPathAsEntered : `'${appPathAsEntered}'`; // Don't tell people to 'cd .'

    Console.info(Console.command("cd " + appPathWithQuotesIfSpaces), Console.options({
      indent: 2
    }));
  }

  Console.info(Console.command("meteor"), Console.options({
    indent: 2
  }));
  Console.info("");
...
${Console.command("meteor build ../output")}`, Console.options({
      indent: 2
    }));
    Console.warn();
  }
  ...
  if (!_.isEmpty(cordovaPlatforms)) {
    let cordovaProject;
    main.captureAndExit('', () => {
      let ensureDevBundleDependencies, pluginVersionsFromStarManifest, displayNameForPlatform;
      module.watch(require("../cordova"), {
        ensureDevBundleDependencies(v) {
          ensureDevBundleDependencies = v;
        },
  ...
  if (testAppPath) {
    try {
      if (files.mkdir_p(testAppPath, 0o700)) {
        ...
        Console.error('The specified --test-app-path directory could not be used, as ' + `"${testAppPath}" already exists and it is not a directory.`);
        return 1;
      }
    } catch (error) {
      Console.error('Unable to create the specified --test-app-path directory of ' + `"${testAppPath}".`);
      throw error;
    }
  }
  ...
  if (options["test-packages"]) {
    ...
    try {
      ...
      if (error.code === 'EACCES' && options['test-app-path']) {
        Console.error('The specified --test-app-path directory of ' + `"${testRunnerAppDir}" exists, but the current user does not have ` + `read/write permission in it.`);
      }
...
main.registerCommand({
  ...
  var minutes = options.minutes || 5; // In verbose mode, we let you know what is going on.
  ...
  try {
    maybeLog("Reserving machine ..."); // The server returns to us an object with the following keys:
  ...
  conn.close(); // Possibly, the user asked us to return a JSON of the data and is going to process it
  ...
  files.writeFile(idpath, ret.sshKey, {
    encoding: 'utf8',
    mode: 0o400
  ...
  files.writeFile(hostpath, addendum, 'utf8'); // Finally, connect to the machine.
  ...
  var sshCommand = child_process.spawn("ssh", connOptions, {
    stdio: 'inherit'
