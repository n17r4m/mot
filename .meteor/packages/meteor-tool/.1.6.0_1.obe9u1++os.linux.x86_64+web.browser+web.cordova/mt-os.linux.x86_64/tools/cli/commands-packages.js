...
module.watch(require("../packaging/updater.js"), {
...
main.registerCommand({
  ...
  if (binary && options['existing-version']) {
    ...
    Console.warn();
    Console.warn("You're not done publishing yet! This package contains binary code and", "must be built on all of Meteor's architectures, including this", "machine's architecture.");
    Console.warn();
    Console.info("You can access Meteor provided build machines, pre-configured to", "support older versions of MacOS and Linux, by running:");

    _.each(["os.osx.x86_64", "os.linux.x86_64", "os.linux.x86_32", "os.windows.x86_32"], function (a) {
      Console.info(Console.command("meteor admin get-machine " + a), Console.options({
        indent: 2
      }));
    });
    ...
    Console.info();
    Console.info(Console.command("meteor publish-for-arch " + packageSource.name + "@" + packageSource.version), Console.options({
      indent: 2
    }));
    Console.info();
    Console.info("For more information on binary ABIs and consistent builds, see:");
    Console.info(Console.url("https://github.com/meteor/meteor/wiki/Build-Machines"), Console.options({
      indent: 2
    }));
    Console.info();
      ...
      _.each(allPackages, function (packageName) {
        buildmessage.enterJob("checking consistency of " + packageName, function () {
          ...
          if (!oldVersionRecord) {
            ...
            var existingBuild = catalog.official.getBuildWithPreciseBuildArchitectures(oldVersionRecord, isopk.buildArchitectures());
...
var maybeUpdateRelease = function (options) {
  ...
  var releaseName = `${releaseTrack}@${releaseVersion}`; // We could at this point springboard to solutionRelease (which is no newer
