...
///////////////////////////////////////////////////////////////////////////////
                                                                             //
// RegExp matching strings that don't start with a `.` or a `/`.             // 1
var topLevelIdPattern = /^[^./]/;                                            // 2
                                                                             // 3
// This function will be called whenever a module identifier that hasn't     // 4
// been installed is required. For backwards compatibility, and so that we   // 5
// can require binary dependencies on the server, we implement the           // 6
// fallback in terms of Npm.require.                                         // 7
makeInstallerOptions.fallback = function (id, parentId, error) {             // 8
  // For simplicity, we honor only top-level module identifiers here.        // 9
  // We could try to honor relative and absolute module identifiers by       // 10
  // somehow combining `id` with `dir`, but we'd have to be really careful   // 11
  // that the resulting modules were located in a known directory (not       // 12
  // some arbitrary location on the file system), and we only really need    // 13
  // the fallback for dependencies installed in node_modules directories.    // 14
  if (topLevelIdPattern.test(id)) {                                          // 15
    if (typeof Npm === "object" &&                                           // 16
        typeof Npm.require === "function") {                                 // 17
      return Npm.require(id, error);                                         // 18
    }                                                                        // 19
  }                                                                          // 20
                                                                             // 21
  throw error;                                                               // 22
                                                                             ...
                                                                             // 24
makeInstallerOptions.fallback.resolve = function (id, parentId, error) {     // 25
  if (topLevelIdPattern.test(id)) {                                          // 26
    // Allow any top-level identifier to resolve to itself on the server,    // 27
    // so that makeInstallerOptions.fallback has a chance to handle it.      // 28
    return id;                                                               // 29
  }                                                                          // 30
                                                                             // 31
  throw error;                                                               // 32
                                                                             ...
                                                                             // 34
meteorInstall = makeInstaller(makeInstallerOptions);                         // 35
var Module = meteorInstall.Module;                                           // 36
                                                                             // 37
Module.prototype.useNode = function () {                                     // 38
  if (typeof npmRequire !== "function") {                                    // 39
    // Can't use Node if npmRequire is not defined.                          // 40
    return false;                                                            // 41
  }                                                                          // 42
                                                                             // 43
  var parts = this.id.split("/");                                            // 44
  var start = 0;                                                             // 45
  if (parts[start] === "") ++start;                                          // 46
  if (parts[start] === "node_modules" &&                                     // 47
      parts[start + 1] === "meteor") {                                       // 48
    start += 2;                                                              // 49
  }                                                                          // 50
                                                                             // 51
  if (parts.indexOf("node_modules", start) < 0) {                            // 52
    // Don't try to use Node for modules that aren't in node_modules         // 53
    // directories.                                                          // 54
    return false;                                                            // 55
  }                                                                          // 56
                                                                             // 57
  try {                                                                      // 58
    npmRequire.resolve(this.id);                                             // 59
  } catch (e) {                                                              // 60
    return false;                                                            // 61
  }                                                                          // 62
                                                                             // 63
  this.exports = npmRequire(this.id);                                        // 64
                                                                             // 65
  return true;                                                               // 66
};                                                                           // 67
                                                                             // 68
