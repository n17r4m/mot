...
async def main(args):
    ...
    if len(args) < 2:
        print(
            """path/to/linescan/data/ 2018-05-22 Name-of_experiment "Notes. Notes." /dump/to/dir"""
        )
    else:
...
async def detect_linescan(path, dump_path=None, *args):
    ...
    if not os.path.isdir(dump_path):
        try:
            ...
            raise ValueError(
                "Data dump directory {} could not be created".format(dump_path)
            )
    ...
    for p in ls.props:
        ...
        if w >= 5.0:
            ...
            areas_circ.append(
                ((d / 2.0) ** 2) * np.pi + np.random.uniform(-np.pi / 2., np.pi / 2.)
            )
            diameters.append(d)
    ...
    Ap_x_V_ax1.hist2d(
        areas_prop[mask], vels[mask], 150, ((0, 25e4), (0, 0.1)), norm=colors.LogNorm()
    )
    ...
    Ac_x_V_ax1.hist2d(
        areas_circ[mask], vels[mask], 150, ((0, 25e4), (0, 0.1)), norm=colors.LogNorm()
    )
    ...
    D_x_V_ax1.hist2d(
        diameters[mask], vels[mask], 150, ((0, 700), (0, 0.1)), norm=colors.LogNorm()
    )
    ...
    E_x_V_ax1.hist2d(
        eccentricities[mask], vels[mask], 150, ((0, 1), (0, 0.1)), norm=colors.LogNorm()
    )
    ...
    with open(dump_path / "detections.csv", "w") as detections:
        ...
        for p in ls.props:
            ...
            if w >= 17.9:
                ...
                v = (
                    LS_timeCal
                    * ((d / h) + np.random.uniform(-LS_timeCal / 2., LS_timeCal / 2.))
                    / 1000000.0
                )

                io.imsave(
                    dump_path / "crops" / str(frame) / "{}-{}.png".format(uid, w),
                    collage,
                    "pil",
                )
