...
async def detect_linescan(path, date=None, name=None, notes="", *args):
    ...
    db_segments = DB_CSV("Segment", ("segment", "experiment", "number"))
    db_frames = DB_CSV("Frame", ("frame", "experiment", "segment", "number"))
    db_particles = DB_CSV(
        "Particle",
        (
            "particle",
    ...
    db_tracks = DB_CSV("Track", ("track", "frame", "particle", "location", "bbox"))
    ...
    try:
            ...
            for p in ls.props:
                ...
                if w >= LS_minWidth / LS_pixelCal:
                    ...
                    if not frame_uuid["added"]:
                        ...
                        db_segments.insert((frame_uuid["uuid"], experiment_uuid, frame))
                        db_frames.insert(
                            (
                                frame_uuid["uuid"],
                                experiment_uuid,
        ...
        db_segments.cleanup()
        db_frames.cleanup()
        db_particles.cleanup()
        db_tracks.cleanup()

    else:
        print("Success")
        print("Committing results to database")
        await db_segments.commit(tx)
        await db_frames.commit(tx)
        await db_particles.commit(tx)
        await db_tracks.commit(tx)
...
class DB_CSV:
    ...
    def __init__(self, table, columns):
        ...
        # # stat.S_IRUSR | stat.S_IWUSR | stat.S_IROTH # User RW, Other R
        # os.fchmod(self.fd, 388)
        # self.file = open(self.fd, "w")

        self.fname = "/tmp/{}_{}.pgdata".format(uuid4(), table)
    ...
    async def commit(self, tx, cleanup=True):
        self.file.flush()
        await tx.execute(
            "COPY {} ( {} ) FROM '{}'".format(
                self.table, ", ".join(self.columns), self.fname
            )
        )
        if cleanup:
            self.cleanup()
